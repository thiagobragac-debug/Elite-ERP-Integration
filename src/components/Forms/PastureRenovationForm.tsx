...
export const PastureRenovationForm: React.FC<PastureRenovationFormProps> = ({
  isOpen, onClose, onSubmit, pastoId, initialData, actionId 
}) => {
  ...
  const [etapaData, setEtapaData] = useState({
    data_registro: new Date(Date.now() - new Date().getTimezoneOffset() * 60000).toISOString().split('T')[0],
    maquina_id: '',
    horas_trabalhadas: '',
    custo_hora: '',
    observacoes: '',
  ...
  const custoTotalReforma = useMemo(() => {
    let total = etapasSalvas.reduce((acc, et) => acc + (parseFloat(et.custo_etapa) || 0), 0);
  ...
  return (
    ...
    >
      ...
      <div style={{ marginBottom: '24px', display: 'flex', gap: '16px', flexWrap: 'wrap' }}>
        <div style={{ flex: 1, minWidth: '200px', padding: '16px', background: 'rgba(59, 130, 246, 0.05)', border: '1px solid rgba(59, 130, 246, 0.2)', borderRadius: '12px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <div>
            <span style={{ display: 'block', fontSize: '11px', fontWeight: 700, color: 'hsl(var(--text-muted))', textTransform: 'uppercase', marginBottom: '4px' }}>Custo Total Acumulado</span>
            <span style={{ fontSize: '24px', fontWeight: 900, color: '#3b82f6' }}>{custoTotalReforma.toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' })}</span>
          </div>
        ...
        <div style={{ flex: 1, minWidth: '200px', padding: '16px', background: 'hsl(var(--bg-main))', border: '1px solid hsl(var(--border))', borderRadius: '12px' }}>
          <span style={{ display: 'block', fontSize: '11px', fontWeight: 700, color: 'hsl(var(--text-muted))', textTransform: 'uppercase', marginBottom: '12px' }}>Linha do Tempo da Reforma</span>
          <div style={{ display: 'flex', gap: '4px' }}>
            {ETAPAS_CONFIG.map((et, idx) => {
              const isCompleted = etapasSalvas.some(s => s.tipo_etapa === et.id);
              const isActive = activeEtapa === et.id;
              return (
                <div key={et.id} style={{ flex: 1, height: '6px', borderRadius: '3px', background: isCompleted ? et.color : isActive ? `${et.color}80` : 'hsl(var(--border))', transition: 'all 0.3s' }} title={et.label} />
              )
            })}
          </div>
        </div>
        ...
        <div style={{ flex: 1, background: 'hsl(var(--bg-card))', border: '1px solid hsl(var(--border))', borderRadius: '16px', padding: '24px' }}>
          ...
          <div className="tauze-input-grid grid-col-2" style={{ marginBottom: '24px' }}>
            <div className="tauze-field-group">
              <label className="tauze-label"><Calendar size={14} /> Data da Operação</label>
              <input 
            ...
            {activeEtapa === 'planejamento' && (
              <div className="tauze-field-group">
                <label className="tauze-label"><Target size={14} /> Objetivo</label>
                <select className="tauze-input" value={reforma.objetivo} onChange={(e) => setReforma({...reforma, objetivo: e.target.value})}>
            ...
            {activeEtapa === 'estabelecimento' && (
              <div className="tauze-field-group">
                <label className="tauze-label"><Activity size={14} /> Status da Reforma</label>
                <select className="tauze-input" value={reforma.status} onChange={(e) => setReforma({...reforma, status: e.target.value})}>
          ...
          {activeEtapa === 'planejamento' && (
            <div style={{ background: 'hsl(var(--bg-main))', padding: '16px', borderRadius: '12px', marginBottom: '24px' }}>
              <h4 style={{ margin: '0 0 16px 0', fontSize: '13px', color: 'hsl(var(--text-secondary))', display: 'flex', alignItems: 'center', gap: '8px' }}><FlaskConical size={16}/> Resultados da Análise de Solo</h4>
              <div className="tauze-input-grid grid-col-3">
                <div className="tauze-field-group">
                  <label className="tauze-label">V% (Saturação Base)</label>
                  <input className="tauze-input" type="number" step="0.1" placeholder="Ex: 45.5" value={reforma.analise_v_percent} onChange={e => setReforma({...reforma, analise_v_percent: e.target.value})} />
                </div>
                <div className="tauze-field-group">
                  <label className="tauze-label">Fósforo (mg/dm³)</label>
                  <input className="tauze-input" type="number" step="0.1" placeholder="Ex: 5.2" value={reforma.analise_p_mgdm3} onChange={e => setReforma({...reforma, analise_p_mgdm3: e.target.value})} />
                </div>
                <div className="tauze-field-group">
                  <label className="tauze-label">Cálcio (cmol/dm³)</label>
                  <input className="tauze-input" type="number" step="0.1" placeholder="Ex: 2.1" value={reforma.analise_ca_cmolc} onChange={e => setReforma({...reforma, analise_ca_cmolc: e.target.value})} />
                </div>
              </div>
            </div>
          )}
          ...
          {(activeEtapa === 'planejamento' || activeEtapa === 'estabelecimento') && (
            <div style={{ background: 'hsl(var(--bg-main))', padding: '16px', borderRadius: '12px', marginBottom: '24px' }}>
              <h4 style={{ margin: '0 0 16px 0', fontSize: '13px', color: 'hsl(var(--text-secondary))', display: 'flex', alignItems: 'center', gap: '8px' }}><Camera size={16}/> Laudo Fotográfico</h4>
              <div className="tauze-input-grid grid-col-2">
                <div className="tauze-field-group">
                  <label className="tauze-label">URL Foto Antes (Planejamento)</label>
                  <input className="tauze-input" type="url" placeholder="https://..." value={reforma.foto_antes_url} onChange={e => setReforma({...reforma, foto_antes_url: e.target.value})} />
                </div>
                {activeEtapa === 'estabelecimento' && (
                  <div className="tauze-field-group">
                    <label className="tauze-label">URL Foto Depois (Conclusão)</label>
                    <input className="tauze-input" type="url" placeholder="https://..." value={reforma.foto_depois_url} onChange={e => setReforma({...reforma, foto_depois_url: e.target.value})} />
                  </div>
                )}
              </div>
            </div>
          ...
          {['correcao', 'preparo', 'controle'].includes(activeEtapa) && (
            <div style={{ background: 'hsl(var(--bg-main))', padding: '16px', borderRadius: '12px', marginBottom: '24px', borderLeft: '3px solid #f59e0b' }}>
              <h4 style={{ margin: '0 0 16px 0', fontSize: '13px', color: '#b45309', display: 'flex', alignItems: 'center', gap: '8px' }}><Tractor size={16}/> Custos de Maquinário (Hora-Máquina)</h4>
              <div className="tauze-input-grid grid-col-3">
                ...
                <div className="tauze-field-group">
                  ...
                  <input 
                    ...
                    onChange={e => setEtapaData({...etapaData, horas_trabalhadas: e.target.value})} 
                    disabled={!etapaData.maquina_id}
                  />
                </div>
              </div>
              {custoMaquinaCurrentEtapa > 0 && (
                <div style={{ marginTop: '12px', fontSize: '12px', color: 'hsl(var(--text-muted))', textAlign: 'right' }}>
                  Custo Maquinário: <strong style={{ color: 'hsl(var(--text-main))' }}>{custoMaquinaCurrentEtapa.toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' })}</strong>
                </div>
              )}
            </div>
          )}
          ...
          <div style={{ marginBottom: '24px' }}>
            <h4 style={{ margin: '0 0 8px 0', fontSize: '13px', color: 'hsl(var(--text-secondary))', display: 'flex', alignItems: 'center', gap: '8px' }}>
              <Layers size={16}/> Baixa de Insumos (Sementes, Adubos, Defensivos)
            </h4>
            <p style={{ fontSize: '11px', color: 'hsl(var(--text-muted))', marginBottom: '16px' }}>Os insumos adicionados aqui serão descontados do Estoque e somados ao custo da Reforma.</p>
            <ConsumptionCart 
              items={items}
              onChange={setItems}
              mode="consumption"
          ...
          <div className="tauze-field-group">
            <label className="tauze-label"><FileText size={14} /> Observações da Etapa</label>
            <textarea 
