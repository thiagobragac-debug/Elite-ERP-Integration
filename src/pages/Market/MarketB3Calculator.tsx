...
export const MarketB3Calculator: React.FC = () => {
  ...
  // Fetch Boi Gordo Cepea with 1 hour staleTime (Requirement 20.5)
  const { data: boiGordoData } = useBoiGordoCepea();
  const physicalPrice = boiGordoData ? Number(boiGordoData.value) : null;

  useEffect(() => {
    fetchB3Prices();
  }, []);

  const fetchB3Prices = async () => {
    setLoading(true);
    try {
      // Fetch B3 Future price via Edge Function
      ...
      // Merge data for chart - fetch historical Cepea data for comparison
      if (b3Quotes.length > 0) {
  ...
  const sparklineFisico = chartData
    .filter((d) => d.Fisico != null)
    .slice(-20)
    .map((d) => ({ value: Number(d.Fisico), label: d.displayDate }));
  const sparklineB3 = chartData
    .filter((d) => d.B3 != null)
    .slice(-20)
    .map((d) => ({ value: Number(d.B3), label: d.displayDate }));
    ...
    <div className="admin-intelligence-page animate-slide-up">
        ...
        <div className="header-brand-group">
          <Breadcrumb
            paths={[
              ...
              { label: 'Calculadora de Base' },
            ]}
          />
          <h1 className="page-title">Calculadora de Base</h1>
          <p className="page-subtitle">
            Cruzamento do Mercado Físico (CEPEA) vs Mercado Futuro (B3)
          </p>
        </div>
      </header>
      ...
      <div className="next-gen-kpi-grid" style={{ gridTemplateColumns: 'repeat(4, 1fr)' }}>
        ...
        >
          ...
          <h2
            style={{
              fontSize: '1.8rem',
              margin: '8px 0',
              color: basis !== null && basis >= 0 ? '#10b981' : '#ef4444',
            }}
          >
          ...
          >
            ...
            {basis !== null && basis >= 0
              ? 'Físico está pagando Prêmio'
              : 'Físico está com Deságio'}
          </div>
        </div>
        <div
          className="tauze-kpi-card"
        ...
        >
          ...
          >
            {basis !== null && futurePrice
              ? `${((Math.abs(basis) / futurePrice) * 100).toFixed(2)}%`
              : '---'}
          </h2>
          <div
            style={{
        ...
        <div style={{ display: 'flex', gap: '20px', alignItems: 'flex-end', flexWrap: 'wrap' }}>
          <div style={{ flex: 1, minWidth: '200px' }}>
            ...
            <div style={{ display: 'flex', gap: '12px' }}>
              ...
              <button
                ...
                onClick={() => {
                  ...
                  navigate('/vendas/contrato', {
                    state: { createHedge: true, b3Ticker, futurePrice },
                  });
                }}
            ...
            <p style={{ fontSize: '11px', color: 'hsl(var(--text-muted))', marginTop: '8px' }}>
              Dica: BGI é o código do Boi Gordo. Letra é o mês de vencimento. Número é o ano.
            </p>
          </div>
        </div>
      </div>
