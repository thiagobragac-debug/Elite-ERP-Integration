...
export const LCDPRPage: React.FC = () => {
  ...
  const activeTab =
    (searchParams.get('tab') as 'lancamentos' | 'resumo' | 'gerar') || 'lancamentos';
  ...
  const [form, setForm] = useState({
    fazenda_id: '',
    data_lancamento: new Date(Date.now() - new Date().getTimezoneOffset() * 60000)
      .toISOString()
      .split('T')[0],
    cod_imovel: '',
    cod_conta_bancaria: '',
  ...
  const openNew = () => {
    ...
    setForm({
      ...
      data_lancamento: new Date(Date.now() - new Date().getTimezoneOffset() * 60000)
        ...
        .split('T')[0],
      cod_imovel: '',
      cod_conta_bancaria: contas[0]?.id || '999',
      tipo: 'R',
      cod_natureza: '01',
      descricao: '',
      cpf_cnpj_participante: '',
    ...
    mutationFn: async (payload: any) => {
      if (editingItem) {
        const { error } = await supabase
          .from('lcdpr_lancamentos')
          .update(payload)
          .eq('id', editingItem.id).eq('tenant_id', tenant?.id);
        if (error) {
          throw error;
        }
      } else {
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    const payload = {
      ...
      cod_imovel:
        form.cod_imovel || (fazenda?.nirf ? `FAZ${fazendas.indexOf(fazenda) + 1}` : 'IMP001'),
  ...
  const deleteMutation = useMutation({
    mutationFn: async (id: string) => {
      const { error } = await supabase.from('lcdpr_lancamentos').delete().eq('id', id).eq('tenant_id', tenant?.id);
      if (error) {
        throw error;
      }
    },
        ...
        <div style={{ display: 'flex' }}>
          <span
            style={{
              ...
              fontSize: 11,
              fontWeight: 800,
              background: l.tipo === 'R' ? '#10b98118' : '#ef444418',
      ...
      accessor: (l: any) => {
        ...
        return (
          <div style={{ fontSize: 12 }}>
            ...
            <span style={{ color: 'hsl(var(--text-muted))', marginLeft: 6, fontSize: 11 }}>
              {nat?.descricao.slice(0, 28)}...
            </span>
          </div>
        );
      },
    ...
    <div className="admin-page animate-slide-up">
      ...
      <header className="page-header">
        <div className="header-brand-group">
          <Breadcrumb
            paths={[{ label: 'Financeiro & Banco', href: '/financeiro' }, { label: 'LCDPR' }]}
          />
      ...
      <div className="next-gen-kpi-grid" style={{ marginBottom: 24 }}>
        {[
          ...
          },
          {
            ...
            color: '#6366f1',
            Icon: FileText,
            sub: `Ano ${anoCalendario}`,
            sparkline: resumoMensal.map((m) => ({ value: m.count, label: m.mes })),
          },
      ...
      <div className="tauze-controls-row" style={{ marginBottom: 20 }}>
        <div className="tauze-tab-group">
          {(
            ...
            <button
              key={id}
              className={`tauze-tab-item ${activeTab === id ? 'active' : ''}`}
        ...
        <div
          style={{
            display: 'flex',
            alignItems: 'center',
            marginLeft: activeTab !== 'lancamentos' ? 'auto' : 16,
          }}
        >
          <span
            style={{
            ...
            {(['TODOS', 'R', 'D'] as const).map((t) => (
              <button
                ...
                style={{
                  padding: '6px 14px',
                  borderRadius: 8,
                  border: `1px solid ${filterTipo === t ? (t === 'R' ? '#10b981' : t === 'D' ? '#ef4444' : 'hsl(var(--brand))') : 'hsl(var(--border))'}`,
                  background:
                    filterTipo === t
                      ? t === 'R'
                        ? '#10b98115'
              ...
              {resumoMensal.map((m, i) => {
                ...
                return (
                  <tr
                    ...
                    style={{
                      borderBottom: '1px solid hsl(var(--border))',
                      background: i % 2 === 0 ? 'transparent' : 'hsl(var(--bg-main)/0.3)',
                      opacity: m.count === 0 ? 0.4 : 1,
                    }}
                  >
                    <td style={{ padding: '12px 16px', fontWeight: 800, fontSize: 13 }}>
                      {m.mes}/{anoCalendario}
                    </td>
                    <td
                      style={{
                    ...
                    <td
                      style={{
                        ...
                        fontSize: 13,
                        fontWeight: 800,
                        color: resultado >= 0 ? '#10b981' : '#ef4444',
                      }}
                    >
                      {resultado >= 0 ? '+' : ''}
                      {fmtBRL(resultado)}
              ...
              >
                ...
                <span
                  style={{
                    fontSize: 13,
                    fontWeight: 600,
                    color: ok ? 'hsl(var(--text-main))' : '#f59e0b',
                  }}
                >
                  {label}
                </span>
          ...
          <div style={{ display: 'flex', gap: 12 }}>
            {(['R', 'D'] as const).map((t) => (
              <button
                ...
                key={t}
                onClick={() =>
                  setForm((f) => ({ ...f, tipo: t, cod_natureza: t === 'R' ? '01' : '11' }))
                }
                style={{
                  flex: 1,
                  padding: '12px 0',
                  borderRadius: 12,
                  border: `2px solid ${form.tipo === t ? (t === 'R' ? '#10b981' : '#ef4444') : 'hsl(var(--border))'}`,
                  background:
                    form.tipo === t ? (t === 'R' ? '#10b98112' : '#ef444412') : 'transparent',
                  fontWeight: 800,
                  fontSize: 14,
                  cursor: 'pointer',
                  color:
            ...
            <div className="tauze-input-grid">
              ...
              <div className="form-group">
                ...
                <select
                  ...
                  onChange={(e) => setForm((f) => ({ ...f, fazenda_id: e.target.value }))}
                >
            ...
            <div className="tauze-input-grid">
              <div className="form-group" style={{ gridColumn: '1/-1' }}>
                ...
                <select
                  ...
                  onChange={(e) => setForm((f) => ({ ...f, cod_natureza: e.target.value }))}
                >
