...
export const DietForm: React.FC<DietFormProps> = ({
  ...
  React.useEffect(() => {
    ...
    if (initialData) {
      setFormData({
        ...
        custo_por_kg: initialData.custo_por_kg
          ? initialData.custo_por_kg.toString().replace(/[^\d.-]/g, '')
          : '0',
        percentual_ms: initialData.percentual_ms
          ? initialData.percentual_ms.toString().replace(/[^\d.-]/g, '')
          : '88',
  ...
  useEffect(() => {
    if (ingredients.length > 0) {
      const totalCost = ingredients.reduce(
        (sum, item) => sum + (item.custo_medio || 0) * (item.quantidade || 0),
        0
      );
  ...
  const dietStats = useMemo(() => {
    ...
    const badges = [];
    if (pb > 25) {
      badges.push({ icon: Flame, text: 'Alto Teor Proteico', color: '#eab308', bg: '#fef08a' });
    }
    if (ndt > 75) {
      badges.push({ icon: Zap, text: 'Super Energética', color: '#8b5cf6', bg: '#ede9fe' });
    }
    if (ms < 35) {
      badges.push({ icon: Droplets, text: 'Alta Umidade', color: '#3b82f6', bg: '#dbeafe' });
    }
    ...
    <SidePanel
      ...
      subtitle={
        initialData
          ...
          : 'Defina os ingredientes e parâmetros nutricionais.'
      }
      icon={Utensils}
      loading={loading}
    ...
    >
      <section className="tauze-form-section">
        ...
        <div className="tauze-input-grid grid-col-1">
          <div style={{ display: 'grid', gridTemplateColumns: '3fr 1fr', gap: '16px' }}>
            <div className="tauze-field-group">
              <label className="tauze-label">
                <Utensils size={14} /> Nome da Dieta
              </label>
              <input
          ...
          <div className="tauze-field-group">
            <label className="tauze-label">
              <Wheat size={14} /> Tipo de Formulação
            </label>
            <div
      ...
      <section className="tauze-form-section">
        ...
        <div className="tauze-input-grid grid-col-2" style={{ marginBottom: '24px' }}>
          {formData.tipo === 'Sal Mineral' ? (
            <div className="tauze-field-group">
              <label className="tauze-label">
                <DollarSign size={14} /> Custo por Kg (R$) - Automático
              </label>
              <input
                className="tauze-input"
                type="number"
            ...
            <>
              <div className="tauze-field-group">
                <label className="tauze-label">
                  <DollarSign size={14} /> Custo Matéria Natural (R$/Kg) - Auto
                </label>
                <input
                  className="tauze-input"
                  type="number"
        ...
        <div className="tauze-input-grid grid-col-3">
          ...
          <div className="tauze-field-group">
            <label className="tauze-label">
              ...
              {formData.tipo === 'Sal Mineral' ? 'g/dia' : '% do Peso Vivo'})
            </label>
            <input
              className="tauze-input"
              type="number"
              step="0.1"
              placeholder={formData.tipo === 'Sal Mineral' ? 'Ex: 120' : 'Ex: 2.2'}
