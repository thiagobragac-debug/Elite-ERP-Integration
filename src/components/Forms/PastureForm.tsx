...
export const PastureForm: React.FC<PastureFormProps> = ({
  ...
  const fetchFarms = async () => {
    try {
      const { data } = await supabase.from('fazendas').select('id, nome');
      if (data) {
        setFarms(data);
      }
    } catch (err) {
  ...
  const totalCapacity = React.useMemo(() => {
    ...
    if (!isNaN(a) && !isNaN(cap) && a > 0) {
      return (a * cap).toFixed(1);
    }
    return null;
  }, [formData.area, formData.capacidade_ua]);

  const daysSinceFertilization = React.useMemo(() => {
    if (!formData.data_ultima_fertilizacao) {
      return null;
    }
    ...
    >
        ...
        <div className="tauze-input-grid grid-col-3">
          <div className="tauze-field-group">
            <label className="tauze-label">
              <Map size={14} /> Selecionar Fazenda / Unidade
            </label>
            <SearchableSelect
          ...
          <div className="tauze-field-group">
            ...
            >
              <span>
                <Activity size={14} /> Capacidade (UA/ha)
              </span>
              {totalCapacity && (
                <span
          ...
          <div className="tauze-field-group">
            <label className="tauze-label">
              <Map size={14} /> Coordenadas (Lat/Long)
            </label>
            <input
              className="tauze-input"
              type="text"
      ...
      <section className="tauze-form-section">
        ...
        <div className="tauze-input-grid grid-col-3">
          ...
          <div className="tauze-field-group">
            <label className="tauze-label">
              <Map size={14} /> Sistema de Pastejo
            </label>
            <SearchableSelect
          ...
          <div className="tauze-field-group">
            <label className="tauze-label">
              <Calendar size={14} /> Data da Última Fertilização
            </label>
            <DateInput
              className="tauze-input"
              type="date"
            ...
            />
            {daysSinceFertilization !== null &&
              daysSinceFertilization < 30 &&
              daysSinceFertilization >= 0 && (
                ...
                >
                  <AlertTriangle size={14} /> Atenção: Área em provável período de carência química
                  ({daysSinceFertilization} dias).
                </div>
              )}
          </div>
          ...
          <div className="tauze-field-group" style={{ gridColumn: 'span 3' }}>
            ...
            {needsAttention && formData.status === 'grazing' && (
              ...
              >
                <AlertTriangle size={14} /> Sugestão: Devido às condições sanitárias ou de
                estrutura, considere alterar para "Descanso" ou "Reforma".
              </div>
            )}
          </div>
        </div>
      ...
      <section className="tauze-form-section">
        ...
        <div className="tauze-input-grid grid-col-3">
          ...
          <div className="tauze-field-group">
            <label className="tauze-label">
              <Activity size={14} /> Recurso Hídrico
            </label>
            <SearchableSelect
              value={formData.agua}
      ...
      <section className="tauze-form-section">
        ...
        <div className="tauze-input-grid grid-col-3">
          <div className="tauze-field-group">
            <label className="tauze-label">
              <Shield size={14} /> Estado da Cerca
            </label>
            <SearchableSelect
          ...
          <div className="tauze-field-group">
            <label className="tauze-label">
              <Flame size={14} /> Plantas Daninhas / Invasoras
            </label>
            <SearchableSelect
          ...
          <div className="tauze-field-group" style={{ gridColumn: 'span 3' }}>
            <label className="tauze-label">
              <Tag size={14} /> Observações Técnicas
            </label>
            <textarea
      ...
      {formData.area && formData.capacidade_ua && (
        ...
        >
          ...
          <div style={{ flex: 1 }}>
            ...
            >
              O pasto{' '}
              <strong style={{ color: 'hsl(var(--text-main))' }}>{formData.nome || '___'}</strong>{' '}
              possui{' '}
              <strong style={{ color: 'hsl(var(--text-main))' }}>{formData.area} hectares</strong>.
              Com uma lotação estipulada de {formData.capacidade_ua} UA/ha, a área pode suportar
              fisicamente até{' '}
              <strong style={{ color: '#10b981' }}>{totalCapacity} Unidades Animais</strong> no
              total. O status atual de manejo está definido como{' '}
              <strong style={{ textTransform: 'uppercase' }}>
                {formData.status === 'grazing'
                  ? 'Em Pastejo'
                  : formData.status === 'resting'
                    ? 'Em Descanso'
                    : formData.status === 'degraded'
                      ? 'Degradado'
                      : 'Em Reforma'}
              </strong>
              .
