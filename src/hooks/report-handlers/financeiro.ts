...
export const fluxoCaixa: ReportHandler = async (tenantId, fazendaId, page = 1, pageSize = 50) => {
  const columns = [
    ...
    {
      header: 'Vencimento / Data',
      accessor: (row: any) => (row.date ? new Date(row.date).toLocaleDateString('pt-BR') : 'N/A'),
    },
    { header: 'Tipo', accessor: (row: any) => (row.type === 'inflow' ? '🟢 Entrada' : '🔴 Saída') },
    {
      header: 'Status',
      accessor: (row: any) => (row.status === 'paid' ? '✅ Liquidado' : '⏳ Pendente'),
    },
  ];
  ...
  try {
    ...
    const [bankRes, payRes, recRes, flowStatsRes] = await Promise.all([
      ...
      withTimeout(
        supabase.rpc('calculate_cash_flow_summary', {
          p_tenant_id: tenantId,
          p_fazenda_id: fazendaId,
        }) as unknown as Promise<any>
      ),
    ...
    const vencidosPagar = (payRes.data || []).filter(
    ...
    return {
      ...
      stats: [
        ...
        },
        {
          ...
          change:
            runway !== '---' ? (Number(runway) > 12 ? 'Excelente' : 'Atenção') : 'Sem burn rate',
 ...
 */
export const contasPagar: ReportHandler = async (
  tenantId,
  fazendaId,
  page = 1,
  pageSize = 25,
  ...
  const columns = [
    ...
    {
      header: 'Fornecedor',
      accessor: (row: any) => row.parceiros?.nome || row.fornecedores?.nome || '---',
    },
    {
      header: 'Valor Total',
      accessor: (row: any) => (row.valor_total ? fmt(Number(row.valor_total)) : 'R$ 0,00'),
    },
    {
      header: 'Vencimento',
      accessor: (row: any) =>
    ...
    {
      header: 'Status',
      accessor: (row: any) => (row.status === 'PAGO' ? '✅ Pago' : '⏳ Pendente'),
    },
  ];
  ...
  try {
    ...
    if (filters.dateStart) {
      query = query.gte('data_vencimento', filters.dateStart);
    }
    if (filters.dateEnd) {
      query = query.lte('data_vencimento', filters.dateEnd);
    }
    ...
    const totalPendente = Number(
      summaryRes.data?.find((s: any) => s.status === 'PENDENTE')?.total_value || 0
    );
    const totalPago = Number(
      summaryRes.data?.find((s: any) => s.status === 'PAGO')?.total_value || 0
    );

    // Calcular vencidos reais nos dados carregados
    const vencidos = bills.filter(
      (b: any) => b.status !== 'PAGO' && b.data_vencimento && b.data_vencimento < today
    ).length;
      ...
      stats: [
        {
          ...
          subtitle: _nextDuePagar
            ? `Próx. vencimento: ${_nextDuePagar}`
            : `Posição em ${todayBR()}`,
          sparkline: spBills,
        ...
        },
        {
          ...
          subtitle: _lastPaidPagar
            ? `Último pagamento: ${_lastPaidPagar}`
            : `Histórico até ${todayBR()}`,
          sparkline: spBills,
 ...
 */
export const contasReceber: ReportHandler = async (
  tenantId,
  fazendaId,
  page = 1,
  pageSize = 25,
  ...
  const columns = [
    ...
    {
      header: 'Cliente',
      accessor: (row: any) => row.parceiros?.nome || row.clientes?.nome || '---',
    },
    {
      header: 'Valor Total',
      accessor: (row: any) => (row.valor_total ? fmt(Number(row.valor_total)) : 'R$ 0,00'),
    ...
    {
      header: 'Status',
      accessor: (row: any) => (row.status === 'PAGO' ? '✅ Recebido' : '⏳ Pendente'),
    },
  ];
  ...
  try {
    ...
    const eficiencia =
      totalGeral > 0 ? `${((totalRecebido / totalGeral) * 100).toFixed(0)}%` : '---';
    ...
    return {
      ...
      stats: [
        ...
        },
        {
          ...
          subtitle: _lastReceivedDate
            ? `Último recebimento: ${_lastReceivedDate}`
            : `Histórico até ${todayBR()}`,
          sparkline: spRec,
 ...
 */
export const extratoBancario: ReportHandler = async (
  tenantId,
  fazendaId,
  page = 1,
  pageSize = 20
  ...
  try {
    ...
    const liquidezImediata = contas
      .filter(
        (c: any) => c.tipo?.toLowerCase() === 'corrente' || c.tipo?.toLowerCase() === 'checking'
      )
    ...
    return {
      ...
      stats: [
        ...
        },
        {
          ...
          sparkline: spContas,
          value:
            liquidezImediata > 0 ? fmt(liquidezImediata) : saldoTotal > 0 ? fmt(saldoTotal) : '---',
          change: liquidezImediata > 0 ? 'Contas correntes' : 'Ver saldo total',
          trend: 'neutral' as const,
        },
        {
          label: 'Bancos Conectados',
          subtitle: _lastBankDate
            ? `Sincronizado em ${_lastBankDate}`
            : `Sincronizado em ${todayBR()}`,
          sparkline: spContas,
...
export const financeOverview: ReportHandler = async (tenantId, fazendaId) => {
  ...
  try {
    ...
    const overdueReceivable = receivables.filter(
      (r: any) => r.status !== 'PAGO' && r.data_vencimento && r.data_vencimento < today
    ).length;
    ...
    return {
      ...
      stats: [
        ...
        },
        {
          ...
          color: '#3b82f6',
          progress:
            ebitdaEstimado > 0 ? Math.min(100, (ebitdaEstimado / (totalReceivable || 1)) * 100) : 0,
        ...
        },
        {
          ...
          change:
            currentRatio > 0 ? (currentRatio > 1.5 ? 'Saudável ✅' : 'Atenção ⚠️') : 'Sem dados',
          trend:
            currentRatio > 1.5
              ? ('up' as const)
              : currentRatio > 0
