...
export const TenantRegistration: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Validação Global de Segurança para novas contas
    const hasMin8Chars = password.length >= 8;
    const hasSpecialChar = /[!@#$%^&*(),.?":{}|<>]/.test(password);
    const hasNumLetter = /[a-zA-Z]/.test(password) && /[0-9]/.test(password);

    if (!hasMin8Chars || !hasSpecialChar || !hasNumLetter) {
      setErrorMessage("Política de Segurança: A senha deve conter no mínimo 8 caracteres, incluindo letras, números e caracteres especiais.");
      setIsLoading(false);
      return;
    }
    ...
    try {
      ...
      if (error) {
        ...
        if (lowerMsg.includes('invalid') && lowerMsg.includes('email')) {
          ...
          errorMsg =
            'Muitas tentativas em um curto período. Por favor, aguarde alguns minutos antes de tentar novamente.';
        } else if (lowerMsg.includes('database error')) {
          errorMsg =
            'Erro no servidor: não foi possível finalizar seu cadastro. Tente novamente mais tarde.';
        } else {
      ...
      >
        ...
        <div style={{ position: 'relative', zIndex: 1 }}>
          ...
          >
            ...
            >
              Transforme o futuro da
              <br />
              <span
                style={{
                  background: 'linear-gradient(135deg, #00b865, #34d399, #0ea5e9)',
                  backgroundClip: 'text',
        ...
        >
          ...
          >
            <div style={{ fontSize: 13, color: 'rgba(255,255,255,0.5)' }}>
              ...
              <Link
                to="/login"
                style={{ color: '#00b865', fontWeight: 600, textDecoration: 'none' }}
              >
                Fazer login
              </Link>
            </div>
