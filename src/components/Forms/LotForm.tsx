...
export const LotForm: React.FC<LotFormProps> = ({
    ...
    if (initialData) {
      setFormData({
        ...
        capacidade: initialData.capacidade
          ? initialData.capacidade.toString().replace(/[^\d.-]/g, '')
          : '',
        data_inicio:
          initialData.data_inicio ||
          new Date(Date.now() - new Date().getTimezoneOffset() * 60000).toISOString().split('T')[0],
        data_fim_prevista: initialData.data_fim_prevista || '',
        dias_ciclo: initialData.dias_ciclo
          ? initialData.dias_ciclo.toString().replace(/[^\d.-]/g, '')
          : '',
        peso_entrada: initialData.peso_entrada
          ? initialData.peso_entrada.toString().replace(/[^\d.-]/g, '')
          : '',
        gmd_alvo: initialData.gmd_alvo
          ? initialData.gmd_alvo.toString().replace(/[^\d.-]/g, '')
          : '',
        peso_alvo: initialData.peso_alvo
          ? initialData.peso_alvo.toString().replace(/[^\d.-]/g, '')
          : '',
        ...
        custo_diario: initialData.custo_diario
          ? initialData.custo_diario.toString().replace(/[^\d.-]/g, '')
          : '',
        ...
        meta_rendimento_carcaca: initialData.meta_rendimento_carcaca
          ? initialData.meta_rendimento_carcaca.toString().replace(/[^\d.-]/g, '')
          : '',
        peso_carcaca_alvo: initialData.peso_carcaca_alvo
          ? initialData.peso_carcaca_alvo.toString().replace(/[^\d.-]/g, '')
          : '',
      });
    } else {
      setFormData({
  ...
  const handleDiasCicloChange = (dias: string) => {
    setFormData((prev) => {
      if (!dias) {
        return { ...prev, dias_ciclo: '', data_fim_prevista: '' };
      }
      ...
      return {
        ...prev,
        dias_ciclo: dias,
        data_fim_prevista: dataInicio.toISOString().split('T')[0],
      };
    });
  };
  ...
  const handleDataFimChange = (dataStr: string) => {
    setFormData((prev) => {
      if (!dataStr) {
        return { ...prev, data_fim_prevista: '', dias_ciclo: '' };
      }
      ...
      return {
        ...prev,
        data_fim_prevista: dataStr,
        dias_ciclo: diffDays > 0 ? diffDays.toString() : '',
      };
    });
  };
    ...
    >
      <section className="tauze-form-section">
        ...
        <div className="tauze-input-grid grid-col-2">
          ...
          <div className="tauze-field-group">
            <label className="tauze-label">
              <Tag size={14} /> Finalidade do Lote
            </label>
            <SearchableSelect
      ...
      <section className="tauze-form-section">
        ...
        <div className="tauze-input-grid grid-col-3">
          ...
          <div className="tauze-field-group">
            <label className="tauze-label">
              <MapPin size={14} /> Pasto Inicial (Opcional)
            </label>
            <SearchableSelect
              ...
              options={[
                {
                  ...
                  label: !formData.fazenda_id
                    ...
                    : loadingPastos
                      ? 'Carregando pastos...'
                      : 'Lote de Confinamento / Sem Pasto',
                },
          ...
          <div className="tauze-field-group">
            <label className="tauze-label">
              <CheckCircle2 size={14} /> Prog. de Qualidade
            </label>
            <SearchableSelect
          ...
          <div className="tauze-field-group">
            <label className="tauze-label">
              <TrendingUp size={14} /> Meta de Rendimento (%)
            </label>
            <input
              className="tauze-input"
              type="number"
              ...
              onChange={(e) =>
                setFormData({ ...formData, meta_rendimento_carcaca: e.target.value })
              }
            />
          </div>
        </div>
      ...
      <section className="tauze-form-section">
        ...
        <div className="tauze-input-grid grid-col-3">
          ...
          <div className="tauze-field-group">
            ...
            >
              <span>
                <Fingerprint size={14} /> Rastreabilidade
              </span>
            </label>
            <label
              style={{
                ...
                gap: '10px',
                padding: '9px 12px',
                background: formData.exige_rastreabilidade
                  ? 'hsl(var(--brand)/0.08)'
                  : 'hsl(var(--bg-main))',
                border: `1px solid ${formData.exige_rastreabilidade ? 'hsl(var(--brand)/0.3)' : 'hsl(var(--border))'}`,
                borderRadius: '10px',
                cursor: 'pointer',
                transition: 'all 0.2s',
                marginTop: '2px',
            ...
            >
              <div style={{ flex: 1, display: 'flex', flexDirection: 'column' }}>
                <span
                  style={{
                    fontSize: '12px',
                    fontWeight: 700,
                    color: formData.exige_rastreabilidade
                      ? 'hsl(var(--brand))'
                      : 'hsl(var(--text-main))',
                  }}
                >
                  Exigir SISBOV / Eletrônico
                </span>
                <span style={{ fontSize: '10px', color: 'hsl(var(--text-muted))' }}>
                  Bloqueia animais sem brinco eletrônico
                </span>
              </div>
              <div
                style={{
              ...
              >
                <div
                  style={{
                    ...
                    position: 'absolute',
                    top: '2px',
                    left: formData.exige_rastreabilidade ? '18px' : '2px',
                    transition: 'all 0.2s',
              ...
              <input
                type="checkbox"
                style={{ display: 'none' }}
                checked={formData.exige_rastreabilidade}
                onChange={(e) =>
                  setFormData({ ...formData, exige_rastreabilidade: e.target.checked })
                }
              />
            </label>
          </div>
      ...
      <section className="tauze-form-section">
        ...
        <div className="tauze-input-grid grid-col-2">
          ...
          <div className="tauze-field-group">
            <label className="tauze-label">
              <Activity size={14} /> Status Inicial
            </label>
            <SearchableSelect
      ...
      {(formData.peso_entrada || formData.dias_ciclo || formData.custo_diario) && (
        ...
        >
          ...
          <div style={{ flex: 1 }}>
            ...
            >
              O animal entrará com{' '}
              <strong style={{ color: 'hsl(var(--text-main))' }}>
                {formData.peso_entrada ? `${formData.peso_entrada}kg` : '___kg'}
              </strong>
              , ficará{' '}
              <strong style={{ color: 'hsl(var(--text-main))' }}>
                {formData.dias_ciclo ? `${formData.dias_ciclo} dias` : '___ dias'}
              </strong>{' '}
              no ciclo
              {formData.gmd_alvo ? ` ganhando ${formData.gmd_alvo}kg/dia` : ''}, e sairá com{' '}
              <strong style={{ color: 'hsl(var(--brand))' }}>
                {formData.peso_alvo ? `${formData.peso_alvo}kg` : '___kg'}
              </strong>
              .
              {formData.dias_ciclo && formData.custo_diario ? (
                <>
                  {' '}
                  Ao custo projetado de{' '}
                  <strong style={{ color: '#10b981' }}>
                    {new Intl.NumberFormat('pt-BR', { style: 'currency', currency: 'BRL' }).format(
                      parseFloat(formData.dias_ciclo) * parseFloat(formData.custo_diario)
                    )}{' '}
                    por animal
                  </strong>
                  .
