...
export const auditLogs: ReportHandler = async (tenantId, fazendaId, page = 1, pageSize = 25, filters: any = {}) => {
  const mockData = {
    data: [
      ...
      { id: '2', table_name: 'pesagens', action: 'INSERT', timestamp: new Date().toISOString(), user_name: 'Jo�o Silva', description: 'Lan�ado peso 540 kg para o animal BR 4520' }
    ],
    columns: [
      { header: 'M�dulo', accessor: 'table_name' },
      { header: 'A��o', accessor: 'action' },
    ...
    stats: [
      ...
      { label: 'Alertas Cr�ticos', sparkline: (() => {  const valStr = String('0'); const match = valStr.match(/[0-9]+(?:[.,][0-9]+)?/); const val = match ? parseFloat(match[0].replace(',', '.')) : 0; return [val*0.6, val*0.7, val*0.8, val*0.85, val*0.9, val*0.95, val].map((v,i) => { const formatted = v % 1 === 0 ? v : Number(v.toFixed(1)); return { value: formatted, label: `${formatted}` }; }); })(), value: '0', icon: AlertCircle, color: '#ef4444', progress: 0, change: 'Est�vel', trend: 'neutral' as const, periodLabel: 'High Severity' },
      { label: 'Cobertura Global', sparkline: (() => {  const valStr = String('100%'); const match = valStr.match(/[0-9]+(?:[.,][0-9]+)?/); const val = match ? parseFloat(match[0].replace(',', '.')) : 0; return [val*0.6, val*0.7, val*0.8, val*0.85, val*0.9, val*0.95, val].map((v,i) => { const formatted = v % 1 === 0 ? v : Number(v.toFixed(1)); return { value: formatted, label: `${formatted}%` }; }); })(), value: '100%', icon: CheckCircle2, color: '#10b981', progress: 100, change: 'Todos os m�dulos ativos', trend: 'neutral' as const, periodLabel: 'Real-time Sync' }
    ],
    totalCount: 2
  };
      ...
      stats: [
        {
          ...
          change: 'N�vel Institucional', trend: 'neutral' as const, periodLabel: 'Fidelity Score',
        ...
        {
          label: 'Alertas Cr�ticos', sparkline: (() => {  const valStr = String(String(deletes)); const match = valStr.match(/[0-9]+(?:[.,][0-9]+)?/); const val = match ? parseFloat(match[0].replace(',', '.')) : 0; return [val*0.6, val*0.7, val*0.8, val*0.85, val*0.9, val*0.95, val].map((v,i) => { const formatted = v % 1 === 0 ? v : Number(v.toFixed(1)); return { value: formatted, label: `${formatted}` }; }); })(), value: String(deletes), icon: AlertCircle, color: '#ef4444', progress: count ? (deletes / count) * 100 : 0,
          change: deletes > 0 ? 'Exclus�es detectadas' : 'Est�vel', periodLabel: 'High Severity',
        ...
        },
        {
          ...
          change: 'Todos os m�dulos ativos', trend: 'neutral' as const, periodLabel: 'Real-time Sync',
        }
      ],
      totalCount: count || 0
    };
...
export const perfisUsuario: ReportHandler = async (tenantId, fazendaId) => {
  const mockData = {
    data: [
      ...
      { id: 'u2', nome: 'Jo�o Silva', cargo: 'Gerente Operacional', status: 'Ativo' }
    ],
    columns: [
      { header: 'Colaborador', accessor: 'nome' },
      { header: 'Cargo/Fun��o', accessor: 'cargo' },
    ...
    stats: [
      ...
      { label: 'Licen�as Ativas', sparkline: (() => {  const valStr = String('12/15'); const match = valStr.match(/[0-9]+(?:[.,][0-9]+)?/); const val = match ? parseFloat(match[0].replace(',', '.')) : 0; return [val*0.6, val*0.7, val*0.8, val*0.85, val*0.9, val*0.95, val].map((v,i) => { const formatted = v % 1 === 0 ? v : Number(v.toFixed(1)); return { value: formatted, label: `${formatted}` }; }); })(), value: '12/15', change: 'Dispon�vel', trend: 'neutral' as const },
      { label: 'Grupos de Seguran�a', sparkline: (() => {  const valStr = String('3 perfis'); const match = valStr.match(/[0-9]+(?:[.,][0-9]+)?/); const val = match ? parseFloat(match[0].replace(',', '.')) : 0; return [val*0.6, val*0.7, val*0.8, val*0.85, val*0.9, val*0.95, val].map((v,i) => { const formatted = v % 1 === 0 ? v : Number(v.toFixed(1)); return { value: formatted, label: `${formatted}` }; }); })(), value: '3 perfis', change: 'Ativo', trend: 'neutral' as const }
    ]
  };
...
/**
 * Governan�a: Admin Intelligence Overview
 */
export const adminOverview: ReportHandler = async (tenantId, fazendaId) => {
  const mockData = {
    data: [
      ...
      { id: '2', action: 'DELETE', entity: 'lotes', description: 'MOCK: Exclu�do lote LT 03 legado', created_at: new Date().toISOString() }
    ],
    columns: [
      { header: 'A��o', accessor: 'action' },
    ...
    stats: [
      { id: 'governanca', label: 'Score de Governan�a', sparkline: (() => {  const valStr = String('88%'); const match = valStr.match(/[0-9]+(?:[.,][0-9]+)?/); const val = match ? parseFloat(match[0].replace(',', '.')) : 0; return [val*0.6, val*0.7, val*0.8, val*0.85, val*0.9, val*0.95, val].map((v,i) => { const formatted = v % 1 === 0 ? v : Number(v.toFixed(1)); return { value: formatted, label: `${formatted}` }; }); })(), value: '88%', change: 'N�vel Institucional', trend: 'neutral' as const, color: 'hsl(var(--brand))', progress: 88 },
      { id: 'licencas', label: 'Licen�as Ativas', sparkline: (() => {  const valStr = String('12/25'); const match = valStr.match(/[0-9]+(?:[.,][0-9]+)?/); const val = match ? parseFloat(match[0].replace(',', '.')) : 0; return [val*0.6, val*0.7, val*0.8, val*0.85, val*0.9, val*0.95, val].map((v,i) => { const formatted = v % 1 === 0 ? v : Number(v.toFixed(1)); return { value: formatted, label: `${formatted}` }; }); })(), value: '12/25', change: 'Plano Enterprise', trend: 'neutral' as const, color: '#3b82f6', progress: 48 },
      { id: 'alertas', label: 'Alertas de Seguran�a', sparkline: (() => {  const valStr = String('0'); const match = valStr.match(/[0-9]+(?:[.,][0-9]+)?/); const val = match ? parseFloat(match[0].replace(',', '.')) : 0; return [val*0.6, val*0.7, val*0.8, val*0.85, val*0.9, val*0.95, val].map((v,i) => { const formatted = v % 1 === 0 ? v : Number(v.toFixed(1)); return { value: formatted, label: `${formatted}` }; }); })(), value: '0', change: 'Ambiente Seguro', trend: 'neutral' as const, color: '#10b981', progress: 100 },
      { id: 'saude', label: 'Sa�de Operacional', sparkline: (() => {  const valStr = String('94%'); const match = valStr.match(/[0-9]+(?:[.,][0-9]+)?/); const val = match ? parseFloat(match[0].replace(',', '.')) : 0; return [val*0.6, val*0.7, val*0.8, val*0.85, val*0.9, val*0.95, val].map((v,i) => { const formatted = v % 1 === 0 ? v : Number(v.toFixed(1)); return { value: formatted, label: `${formatted}%` }; }); })(), value: '94%', change: 'SLA de Inst�ncia', trend: 'neutral' as const, color: '#f59e0b', progress: 94 }
    ],
    totalCount: 2
  };
  ...
  try {
    ...
    return {
      ...
      stats: [
        {
          id: 'governanca',
          label: 'Score de Governan�a', sparkline: (() => {  const valStr = String('88%'); const match = valStr.match(/[0-9]+(?:[.,][0-9]+)?/); const val = match ? parseFloat(match[0].replace(',', '.')) : 0; return [val*0.6, val*0.7, val*0.8, val*0.85, val*0.9, val*0.95, val].map((v,i) => { const formatted = v % 1 === 0 ? v : Number(v.toFixed(1)); return { value: formatted, label: `${formatted}` }; }); })(), value: '88%', change: 'N�vel Institucional',
        ...
        {
          id: 'alertas',
          label: 'Alertas de Seguran�a', sparkline: (() => {  const valStr = String(criticalEvents); const match = valStr.match(/[0-9]+(?:[.,][0-9]+)?/); const val = match ? parseFloat(match[0].replace(',', '.')) : 0; return [val*0.6, val*0.7, val*0.8, val*0.85, val*0.9, val*0.95, val].map((v,i) => { const formatted = v % 1 === 0 ? v : Number(v.toFixed(1)); return { value: formatted, label: `${formatted}` }; }); })(), value: criticalEvents, change: criticalEvents > 0 ? 'A��o Requerida' : 'Ambiente Seguro',
        ...
        {
          id: 'saude',
          label: 'Sa�de Operacional', sparkline: (() => {  const valStr = String('94%'); const match = valStr.match(/[0-9]+(?:[.,][0-9]+)?/); const val = match ? parseFloat(match[0].replace(',', '.')) : 0; return [val*0.6, val*0.7, val*0.8, val*0.85, val*0.9, val*0.95, val].map((v,i) => { const formatted = v % 1 === 0 ? v : Number(v.toFixed(1)); return { value: formatted, label: `${formatted}%` }; }); })(), value: '94%', change: 'SLA de Inst�ncia',
          trend: 'neutral' as const,
          color: '#f59e0b',
          progress: 94,
