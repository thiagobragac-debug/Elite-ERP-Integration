...
export const ModuleSettings: React.FC = () => {
  ...
  const getActionLabel = () => {
    if (activeModule === 'sistema') {
      if (activeSetting === 'certificados') {
        return 'NOVO CERTIFICADO';
      }
      return systemIsSaving
        ? 'SINCRONIZANDO...'
        : systemSaveSuccess
          ? 'CONFIGURAÇÕES SALVAS'
          : 'SALVAR ALTERAÇÕES';
    }
    if (activeSetting === 'ncms') {
      return 'NOVO NCM';
    }
    ...
    <div className="admin-page animate-slide-up">
      <header className="page-header">
        <div className="header-brand-group">
          ...
          <p className="page-subtitle">
            Centralize parâmetros, categorias e regras fiscais de todos os módulos.
          </p>
        </div>
        <div
        ...
        >
          ...
          <button
            ...
            disabled={
              activeModule === 'sistema' && activeSetting !== 'certificados' && systemIsSaving
            }
          >
            {activeModule === 'sistema' ? (
              activeSetting === 'certificados' ? (
                <Plus size={18} />
              ) : systemIsSaving ? (
      ...
      <div style={{ display: 'flex', gap: '24px', alignItems: 'flex-start' }}>
        ...
        <div style={{ flex: 1, minWidth: 0, display: 'flex', flexDirection: 'column' }}>
          ...
          {activeSetting === 'cargos' && (
            <RoleSettingsTab searchTerm={searchTerm} triggerCreate={triggerCreate} />
          )}
          {activeSetting === 'certificados' && (
            <CertificateSettingsTab searchTerm={searchTerm} triggerCreate={triggerCreate} />
          )}
          {activeSetting === 'categorias' && (
            <CategorySettingsTab
              modulo={activeModule}
          ...
          {activeSetting === 'periodos' && (
            <PeriodManagementTab searchTerm={searchTerm} triggerCreate={triggerCreate} />
          )}
          {activeSetting === 'racas' && (
            <CategorySettingsTab
              modulo="racas"
          ...
          {(activeSetting === 'planos' ||
            activeSetting === 'regras' ||
            activeSetting === 'manutencao') && (
            <div
