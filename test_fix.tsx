...
export const UserManagement: React.FC = () => {
  ...
  // --- M�DULO DE GOVERNAN�!A & SEGURAN�!A (PREMIUM EVOLUTION) ---
      ...
      if (!error && data) {
        ...
        const mappedLogs = data.map((log: any) => {
          ...
          if (act.includes('DELETE') || act.includes('ALERT') || act.includes('FAIL') || act.includes('SECURITY') || details.includes('BLOQUE') || details.includes('ERR')) {
            type = 'CRITICAL';
          } else if (act.includes('UPDATE') || act.includes('WARN') || details.includes('SESSÒO') || details.includes('ALTERA�!ÒO')) {
            type = 'WARN';
          }
                  ...
                  >
                    <div className="card-left-section" style={{ padding: '16px', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center' }}>
                      ...
                      <span className={`status-pill ${user.status === 'active' ? 'active' : 'stopped'}`} style={{ marginTop: '8px', fontSize: '9px' }}>
                        {user.status === 'active' ? '�� Online' : '�9 Offline'}
                      </span>
                    </div>
                ...
                <table className="matrix-table" style={{ width: '100%', borderCollapse: 'collapse', textAlign: 'left', background: 'white' }}>
                  <thead>
                    <tr style={{ background: 'var(--bg-main)', borderBottom: '1px solid var(--border)' }}>
                      <th style={{ padding: '16px 20px', fontSize: '0.75rem', fontWeight: 800, color: 'var(--text-muted)', width: '30%' }}>M�DULO / RECURSO</th>
              ...
              <section className="security-panel">
                ...
                <div className="security-options">
                  ...
                  <button className="maintenance-btn" onClick={() => confirm('Ativar Modo de Manutenção?') && setSaveSuccess(true)}>
                    <Shield size={16} /> MODO DE MANUTEN�!ÒO
                  </button>
