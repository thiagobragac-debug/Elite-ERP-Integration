# Script de Analise de Codigo - Tauze ERP
# Uso: .\scripts\analyze.ps1

Write-Host "=================================" -ForegroundColor Cyan
Write-Host "Analise de Codigo - Tauze ERP" -ForegroundColor Cyan
Write-Host "=================================" -ForegroundColor Cyan
Write-Host ""
...
Write-Host "   Linhas de codigo: $totalLines" -ForegroundColor White
Write-Host ""
...
$coverage = if ($components -gt 0) { [math]::Round(($tests / $components) * 100, 1) } else { 0 }
...
# TODOs e FIXMEs
Write-Host "TODOs e FIXMEs:" -ForegroundColor Yellow
$todos = Select-String -Path .\src\*.tsx,.\src\*.ts -Pattern "TODO|FIXME|XXX|HACK" -Recurse -ErrorAction SilentlyContinue | Measure-Object
...
$score += 25  # Assume dependencies are OK
