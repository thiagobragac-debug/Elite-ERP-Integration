...
export const FuelManagement: React.FC = () => {
  ...
  const { activeFarm, isGlobalMode, activeFarmId, activeTenantId, applyFarmFilter } =
    useFarmFilter();
  ...
  const [isAnalysisOpen, setIsAnalysisOpen] = usePersistentState(
    'FuelManagement_isAnalysisOpen',
    false
  );
  const [showAdvancedFilters, setShowAdvancedFilters] = usePersistentState(
    'FuelManagement_showAdvancedFilters',
    false
  );
  ...
  const handleExport = (format: 'csv' | 'excel' | 'pdf') => {
    const filteredData = logs.filter((l) => {
      ...
      const matchesFuel =
        filterValues.fuelTypes.length === 0 || filterValues.fuelTypes.includes(l.tipo_combustivel);
      ...
      return (
        matchesSearch && matchesTab && matchesStatus && matchesFuel && matchesLiters && matchesDate
      );
    });
    ...
    if (format === 'csv') {
      exportToCSV(exportData, 'log_abastecimento');
    } else if (format === 'excel') {
      exportToExcel(exportData, 'log_abastecimento');
    } else if (format === 'pdf') {
      exportToPDF(exportData, 'log_abastecimento', 'Relatório de Abastecimento de Frota');
    }
  };
  ...
  const handleDelete = async (id: string) => {
    const isConfirmed = await confirm({
      title: 'Atenção',
      description: 'Deseja excluir este abastecimento?',
      ...
      <header className="page-header">
        <div className="header-brand-group">
          ...
          <p className="page-subtitle">
            Telemetria de consumo, análise de autonomia e controle rigoroso de custos energéticos.
          </p>
        </div>
          ...
          <div className="export-dropdown-container">
            ...
            <div id="export-menu-fuel" className="export-menu">
              <button
                onClick={() => {
                  handleExport('csv');
                  document.getElementById('export-menu-fuel')?.classList.remove('active');
                }}
              >
                Excel (.CSV)
              </button>
            ...
            <>
              ...
              >
                ...
                >
                  ...
                  >
                    <DollarSign size={12} /> Gasto Total Frota
                  </span>
                  <div
                    style={{
                      fontSize: '20px',
                ...
                >
                  ...
                  >
                    <Droplets size={12} /> Volume Queimado
                  </span>
                  <div
                    style={{
                      fontSize: '20px',
                  ...
                  >
                    {autonomyAnalysis
                      .reduce((acc, curr) => acc + curr.litrosTotais, 0)
                      .toLocaleString()}{' '}
                    L
                  </div>
                </div>
                ...
                >
                  ...
                  >
                    <TrendingDown size={12} /> Top Despesa
                  </span>
                  <div
                    style={{
                      fontSize: '16px',
                  ...
                  return (
                    <div
                      ...
                      style={{
                        ...
                        padding: '20px',
                        borderRadius: '16px',
                        border: `1px solid ${isGargalo ? '#ef4444' : 'hsl(var(--border))'}`,
                        position: 'relative',
                        overflow: 'hidden',
                      }}
                    >
                        ...
                        <div style={{ textAlign: 'right' }}>
                          <span
                            style={{
                              fontSize: '20px',
                              fontWeight: 900,
                              color: isGargalo ? '#ef4444' : '#1e293b',
                            }}
                          >
                            {m.consumoReal > 0 ? m.consumoReal.toFixed(1) : '---'}{' '}
                            <span
                              style={{
                                fontSize: '12px',
                                fontWeight: 600,
                            ...
                            >
                              {m.unidade_medida === 'horas' ? 'L/h' : 'km/L'}
                            </span>
                          </span>
                        </div>
                      </div>
                      ...
                      >
                        <div>
                          ...
                          <span style={{ fontWeight: 700, color: '#3b82f6' }}>
                            {m.deltaMedidor > 0 ? m.deltaMedidor : '---'}{' '}
                            {m.unidade_medida === 'horas' ? 'h' : 'km'}
                          </span>
                        </div>
                        <div>
                          <label
                          ...
                          <span style={{ fontWeight: 700, color: '#1e293b' }}>
                            {m.litrosTotais.toLocaleString()} L
                          </span>
                        </div>
                        <div>
                          <label
