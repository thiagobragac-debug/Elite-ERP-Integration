...
const promptMenu: Record<string, { label: string; prompts: string[] }> = {
  pecuaria: {
    ...
    prompts: [
      ...
      'Animais com desvio de meta no GMD',
      'Qual a taxa de prenhez do último mês?',
    ],
  },
  mercado: {
    label: '📈 Mercado',
    prompts: [
      'Analisar histórico do Boi Gordo (CEPEA)',
      'Qual a tendência de Sazonalidade da arroba?',
      'Comparar Boi Gordo com Milho (B3)',
    ],
  },
  financeiro: {
    label: '💰 Financeiro',
    prompts: [
      ...
      'Qual meu nível de Inadimplência?',
      'Previsão de Runway e fluxo de caixa',
    ],
  },
  agricultura: {
    label: '🌾 Agricultura',
  ...
  frota: {
    ...
    prompts: [
      ...
      'Disponibilidade de frota atual',
...
export const GlobalCopilot: React.FC = () => {
  ...
  const [chatHistory, setChatHistory] = useState<any[]>([
    {
      type: 'system',
      text: 'Olá! Sou o Tauze Copilot. O novo módulo de Inteligência de Mercado e Gráficos de Previsão já estão conectados ao meu sistema. Como posso ajudar na sua gestão hoje?',
    },
  ]);
  ...
  return (
    <>
        ...
        {isCopilotOpen && (
          ...
          >
            ...
            <div className="copilot-chat">
              ...
              {chatHistory.length === 1 && suggestionCategory === null && (
                <>
                  ...
                  >
                    Escolha um módulo para explorar:
                  </div>
                  {Object.entries(promptMenu).map(([key, item]) => (
                    ...
                    >
                      {item.label}{' '}
                      <ChevronRight size={14} style={{ float: 'right', marginTop: '2px' }} />
                    </div>
                  ))}
                </>
              )}
              {chatHistory.length === 1 && suggestionCategory !== null && (
                <>
                  ...
                  >
                    {promptMenu[suggestionCategory]?.label} - Sugestões:
                  </div>
                  <div
                    className="msg suggestion back-btn"
                    onClick={() => setSuggestionCategory(null)}
                    style={{
            ...
            <div className="copilot-input">
              ...
              />
              <button className="send-btn" onClick={handleCopilotSend}>
      ...
      <button
        className={`copilot-floating-btn ${isMinimized ? 'minimized' : ''}`}
