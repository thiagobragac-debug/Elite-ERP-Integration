...
export const TenantForm: React.FC<TenantFormProps> = ({isOpen, onClose, onSubmit, initialData, availablePlans = [], actionId }) => {
  ...
  const availableModules = [
    { id: 'financial_adv', name: 'Financeiro Avançado', icon: PiggyBank, desc: 'DRE, Fluxo de Caixa e LCDPR' },
    { id: 'b3_calc', name: 'Calculadora B3', icon: MonitorPlay, desc: 'Hedge e mercado futuro' },
    { id: 'custom_reports', name: 'Relatórios Dinâmicos', icon: Database, desc: 'PowerBI Embed e Customizados' }
  ];
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '32px' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '24px' }}>
            <div style={{ padding: '16px', borderRadius: '12px', border: '1px solid hsl(var(--border))', background: 'hsl(var(--bg-main))' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '8px' }}>
                ...
                <label className="tauze-switch">
                  <input 
                    ...
                    onChange={e => setFormData({...formData, infrastructure: e.target.checked ? 'dedicated' : 'shared'})}
              ...
              <p style={{ margin: 0, fontSize: '11px', color: 'hsl(var(--text-muted))' }}>
                {isEnterprise ? 'Ative para alocar DB dedicado.' : 'Disponível apenas em planos Enterprise.'}
              </p>
            </div>
        ...
        {!initialData && (
          <section className="tauze-form-section">
            ...
            <div style={{ background: 'hsl(var(--warning)/0.1)', border: '1px solid hsl(var(--warning)/0.3)', padding: '12px 16px', borderRadius: '10px', display: 'flex', alignItems: 'flex-start', gap: '12px', marginBottom: '24px' }}>
              <ShieldCheck size={18} color="hsl(var(--warning))" style={{ marginTop: '2px' }} />
              <div>
                <p style={{ margin: 0, fontSize: '12px', fontWeight: 700, color: 'hsl(var(--warning))' }}>Segurança de Provisionamento</p>
                <p style={{ margin: '4px 0 0', fontSize: '11px', color: 'hsl(var(--text-main))', opacity: 0.8 }}>
                  O usuário cadastrado receberá um email criptografado com acesso administrativo vitalício à instância.
                </p>
              </div>
            </div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px' }}>
              <div className="tauze-field-group">
                <label style={{ fontSize: '11px', fontWeight: 800, color: 'hsl(var(--text-muted))' }}>
                  NOME DO GESTOR
                </label>
                <input 
                  className="tauze-input"
