...
export const ScaleConfigModal: React.FC<ScaleConfigModalProps> = ({ isOpen, onClose }) => {
  ...
  useEffect(() => {
    ...
    if (status === 'CONNECTED') {
      ...
      setTerminalLogs(prev => [...prev, `[${new Date().toLocaleTimeString()}] LINK ESTABLISHED. WAITING DATA...`]);
  ...
  return (
    <SidePanel
      ...
      icon={Settings2}
      submitLabel={isConnecting ? "Conectando..." : status === 'CONNECTED' ? 'Salvar Configuração' : "Testar Conexão"}
          ...
          <div style={{ display: 'flex', gap: '12px' }}>
            <button 
              ...
              style={{ 
                ...
                gap: '8px', 
                cursor: status === 'CONNECTED' || isConnecting ? 'not-allowed' : 'pointer',
                opacity: status === 'CONNECTED' || isConnecting ? (connectionType === 'BLUETOOTH' ? 1 : 0.5) : 1,
                transition: 'all 0.2s ease',
                boxShadow: connectionType === 'BLUETOOTH' ? '0 6px 16px hsl(var(--brand) / 0.08)' : 'none'
              }}
              onClick={() => { setConnectionType('BLUETOOTH'); setScannedDevice(null); }}
            >
              <Bluetooth size={22} style={{ color: connectionType === 'BLUETOOTH' ? 'hsl(var(--brand))' : 'hsl(var(--text-muted) / 0.8)' }} />
              <span style={{ fontSize: '11px', fontWeight: 800, letterSpacing: '0.5px' }}>BLUETOOTH</span>
            </button>
            <button 
              type="button"
              className="conn-config-btn"
              ...
              style={{ 
                ...
                gap: '8px', 
                cursor: status === 'CONNECTED' || isConnecting ? 'not-allowed' : 'pointer',
                opacity: status === 'CONNECTED' || isConnecting ? (connectionType === 'WIFI' ? 1 : 0.5) : 1,
                transition: 'all 0.2s ease',
                boxShadow: connectionType === 'WIFI' ? '0 6px 16px hsl(var(--brand) / 0.08)' : 'none'
              }}
              onClick={() => setConnectionType('WIFI')}
            >
              <Wifi size={22} style={{ color: connectionType === 'WIFI' ? 'hsl(var(--brand))' : 'hsl(var(--text-muted) / 0.8)' }} />
              <span style={{ fontSize: '11px', fontWeight: 800, letterSpacing: '0.5px' }}>WI-FI / IP</span>
            </button>
            <button 
              type="button"
              className="conn-config-btn"
              ...
              style={{ 
                ...
                gap: '8px', 
                cursor: status === 'CONNECTED' || isConnecting ? 'not-allowed' : 'pointer',
                opacity: status === 'CONNECTED' || isConnecting ? (connectionType === 'USB' ? 1 : 0.5) : 1,
                transition: 'all 0.2s ease',
                boxShadow: connectionType === 'USB' ? '0 6px 16px hsl(var(--brand) / 0.08)' : 'none'
              }}
              onClick={() => setConnectionType('USB')}
            >
              <Usb size={22} style={{ color: connectionType === 'USB' ? 'hsl(var(--brand))' : 'hsl(var(--text-muted) / 0.8)' }} />
              <span style={{ fontSize: '11px', fontWeight: 800, letterSpacing: '0.5px' }}>USB / SERIAL</span>
            </button>
          </div>
        </div>
          ...
          {connectionType === 'BLUETOOTH' && (
            <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                <span style={{ fontSize: '12px', fontWeight: 800, color: 'hsl(var(--text-main))' }}>Dispositivo Pareado</span>
                {status !== 'CONNECTED' && (
                  <button type="button" onClick={handleScanBluetooth} disabled={isScanning} className="glass-btn secondary" style={{ padding: '6px 12px', fontSize: '11px', borderRadius: '8px', display: 'flex', gap: '6px', alignItems: 'center' }}>
                    {isScanning ? <RefreshCw size={14} className="spin-slow" /> : <Search size={14} />}
              ...
              <div style={{ 
                ...
                <Bluetooth size={18} style={{ color: scannedDevice ? 'hsl(var(--brand))' : 'hsl(var(--text-muted))' }} />
                <span style={{ fontSize: '13px', fontWeight: 700, color: scannedDevice ? 'hsl(var(--text-main))' : 'hsl(var(--text-muted) / 0.7)' }}>
                  {scannedDevice || 'Nenhum dispositivo selecionado'}
                </span>
                {scannedDevice && <CheckCircle2 size={16} style={{ color: '#10b981', marginLeft: 'auto' }} />}
              </div>
            </div>
          )}
          ...
          {connectionType === 'USB' && (
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px' }}>
              <div className="tauze-field-group">
                ...
                <select className="tauze-input" value={comPort} onChange={e => setComPort(e.target.value)} disabled={status === 'CONNECTED'}>
              ...
              <div className="tauze-field-group">
                ...
                <select className="tauze-input" value={baudRate} onChange={e => setBaudRate(e.target.value)} disabled={status === 'CONNECTED'}>
        ...
        <div style={{ 
          ...
          {status === 'IDLE' && (
            <div style={{ display: 'flex', alignItems: 'center', gap: '14px', color: 'hsl(var(--text-muted))', padding: '4px 0' }}>
              ...
              <span style={{ fontSize: '13px', fontWeight: 700 }}>Aguardando parâmetros para iniciar pareamento...</span>
            </div>
          )}
          
          {status === 'CONNECTING' && (
            <div style={{ display: 'flex', alignItems: 'center', gap: '14px', color: 'hsl(var(--brand))', padding: '4px 0' }}>
              <RefreshCw size={22} className="spin-slow" />
              <div style={{ display: 'flex', flexDirection: 'column' }}>
                <span style={{ fontSize: '13px', fontWeight: 800 }}>Diagnosticando...</span>
                <span style={{ fontSize: '11px', fontWeight: 600, opacity: 0.8 }}>{connectionStep}</span>
              </div>
            </div>
          )}
          ...
          {status === 'CONNECTED' && (
            <>
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '14px', color: '#10b981', padding: '4px 0' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '14px', flex: 1 }}>
                  <div style={{ position: 'relative' }}>
                    ...
                    <span style={{ position: 'absolute', top: -2, right: -2, width: '8px', height: '8px', borderRadius: '50%', background: '#10b981', boxShadow: '0 0 8px #10b981', animation: 'pulse 1.5s infinite' }} />
                  </div>
                  <div style={{ flex: 1 }}>
                    <div style={{ fontSize: '13.5px', fontWeight: 900, color: 'hsl(var(--text-main))' }}>Ligação Estabelecida ({brand})</div>
                    <div style={{ fontSize: '11.5px', fontWeight: 700, color: '#10b981', marginTop: '2px' }}>Streaming de porta ativo • {connectionType}</div>
                  </div>
                </div>
              ...
              <div style={{ 
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: '8px', padding: '6px 12px', background: '#111', borderBottom: '1px solid #333' }}>
                  <Terminal size={12} color="#888" />
                  <span style={{ fontSize: '9px', fontWeight: 700, color: '#888', textTransform: 'uppercase', letterSpacing: '1px' }}>Raw Data Console</span>
                ...
                <div style={{ 
                  ...
                  {terminalLogs.length === 0 ? (
                    <div style={{ color: '#555', fontStyle: 'italic' }}>Aguardando transmissão...</div>
                  ) : (
          ...
          {status === 'ERROR' && (
            <div style={{ display: 'flex', flexDirection: 'column', gap: '6px', color: '#ef4444', padding: '4px 0' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '14px' }}>
                <AlertCircle size={22} />
                <span style={{ fontSize: '13px', fontWeight: 800 }}>Falha ao localizar dispositivo.</span>
              </div>
              <span style={{ fontSize: '11px', fontWeight: 600, opacity: 0.9, marginLeft: '36px' }}>
                Dica: Verifique se a balança {brand} está no mesmo segmento de rede e configurada no modo Servidor/Streaming.
              </span>
            </div>
          )}
        </div>
