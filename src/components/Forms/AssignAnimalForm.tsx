            ...
            filtered.map((item) => (
              <div
                ...
                style={{
                  ...
                  background: item.id === value ? 'hsl(var(--brand) / 0.08)' : 'white',
                ...
                onMouseLeave={(e) =>
                  (e.currentTarget.style.background =
                    item.id === value ? 'hsl(var(--brand) / 0.08)' : 'white')
                }
              >
                ...
                <div style={{ display: 'flex', gap: '5px', flexShrink: 0, marginLeft: '8px' }}>
                  ...
                  {(item.capacidade || item.capacidade_ua) && (
                    ...
                    >
                      cap. {item.capacidade || item.capacidade_ua}
                    </span>
                  )}
                </div>
              </div>
...
function CapacityBar({
  ...
  const formatNumber = (num: number) =>
    unit === 'UA' ? num.toFixed(1) : Math.round(num).toString();
...
export const AssignAnimalForm: React.FC<AssignAnimalFormProps> = ({
  isOpen,
  onClose,
  onSubmit,
  mode,
  ...
  const destName =
    selectedDestName || destinations.find((d) => d.id === selectedDestination)?.nome || 'â€”';
  ...
  const fetchDestinations = async () => {
    ...
    const { data, error } = isLoteMode ? await filtered.neq('status', 'ARQUIVADO') : await filtered;
    if (error) {
      console.error('[AssignAnimalForm] fetchDestinations error:', error);
    }
    if (data) {
  ...
  const fetchUnassignedAnimals = async () => {
    ...
    try {
      ...
      if (error) {
        console.error('[AssignAnimalForm] fetchUnassignedAnimals error:', error);
      }
      if (data) {
  ...
  const selectAll = () => {
    ...
    } else if (selectedAnimals.length === allowedFiltered.length && allowedFiltered.length > 0) {
      setSelectedAnimals([]);
    } else {
      setSelectedAnimals(allowedFiltered.map((a) => a.id));
    }
  ...
  const categorias = useMemo(
    () => [...new Set(unassignedAnimals.map((a) => a.categoria).filter(Boolean))],
    [unassignedAnimals]
  );
            ...
            >
              ...
              >
                ...
                >
                  ...
                  <Row
                    label="Animais"
                    value={`${selectedAnimals.length} animais selecionados${!isLoteMode && selectedAnimals.length > 0 ? ` (${selectedAddingValue.toFixed(1)} UA)` : ''}`}
                  />
                  <Row label="Motivo" value={motivo} />
                  <Row
                    label="Data"
                    value={new Date(`${movDate}T12:00:00`).toLocaleDateString('pt-BR')}
                  />
                  {afterPct !== null && (
                    <Row
                ...
                >
                  ...
                  {selAnimals.length > 12 && (
                    <span
                      style={{ color: 'hsl(var(--text-muted))', fontSize: '11px', fontWeight: 600 }}
                    >
                      +{selAnimals.length - 12} mais
                    </span>
                  )}
                </div>
            ...
            <div style={{ flex: 1, maxWidth: '400px' }}>
              {destCapacity && (
                <CapacityBar
                  ...
                  adding={selectedAddingValue}
                  unit={isLoteMode ? 'Cab.' : 'UA'}
                />
              )}
            </div>
          ...
          <div className="tauze-input-grid grid-col-3">
            <div className="tauze-field-group">
              <DestSearch
                ...
                label={
                  <>
                    {isLoteMode ? <Layers size={14} /> : <Trees size={14} />} {entityLabel} de
                    Destino
                  </>
                }
              />
                ...
                return (
                  <div style={{ marginTop: '8px' }}>
                    ...
                    {!isLoteMode && (dest.status === 'resting' || dest.status === 'renovation') && (
                      ...
                      >
                        <AlertTriangle size={14} /> Atenção: Área de destino está em{' '}
                        {dest.status === 'resting' ? 'Descanso' : 'Reforma'}.
                      </div>
                    )}
                  </div>
                );
          ...
          <div className="tauze-field-group full-width">
            ...
            <div className="tauze-selection-header">
              <label>
                ...
                {unassignedAnimals.length > 0 && (
                  ...
                  >
                    {selectedAnimals.length}/{filteredAnimals.length} cab.
                    {!isLoteMode && selectedAnimals.length > 0
                      ? ` (${selectedAddingValue.toFixed(1)} UA)`
                      : ''}
                  </span>
                )}
              </label>
            ...
            {unassignedAnimals.length === 0 && !loading ? (
              <div className="assign-empty-state">
                ...
                <p>
                  Todos os animais ativos desta fazenda já estão vinculados a um{' '}
                ...
                <div className="tauze-animal-picker">
                  {loading ? (
                    <div className="picker-loading">
                      Buscando animais sem {entityLabel.toLowerCase()}...
                    </div>
                    ...
                    <div className="picker-list-adv">
                      {filteredAnimals.map((animal) => {
                        ...
                        const isRFIDBlocked =
                          isLoteMode &&
                          targetLot?.exige_rastreabilidade &&
                        ...
                        return (
                          ...
                          >
                            ...
                            <div className="p-info-row">
                              ...
                              {animal.categoria && (
                                <span className="p-tag">{animal.categoria}</span>
                              )}
                              {animal.sexo && (
                                <span
                                  className="p-tag"
