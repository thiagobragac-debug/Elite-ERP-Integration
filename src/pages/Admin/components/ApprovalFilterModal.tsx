        ...
        <div className="tauze-sidebar-body">
          ...
          <div className="tauze-filter-section">
            ...
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: '8px' }}>
              {isPendencies ? (
                <>
                  <button
                    className={`tauze-tag-chip ${filters.status === 'all' ? 'active' : ''}`}
                    onClick={() => setFilters({ ...filters, status: 'all' })}
                    style={{
                      flex: '1 1 45%',
                      borderColor: filters.status === 'all' ? '#3b82f6' : 'hsl(var(--border))',
                      background: filters.status === 'all' ? '#3b82f6' : 'hsl(var(--bg-card))',
                      color: filters.status === 'all' ? 'white' : 'hsl(var(--text-muted))',
                    }}
                  >
                    Todos
                  </button>
                  <button
                    ...
                    style={{
                      flex: '1 1 45%',
                      borderColor: filters.status === 'pending' ? '#3b82f6' : 'hsl(var(--border))',
                      background: filters.status === 'pending' ? '#3b82f6' : 'hsl(var(--bg-card))',
                  ...
                  <button
                    className={`tauze-tag-chip ${filters.status === 'approved' ? 'active' : ''}`}
                    onClick={() => setFilters({ ...filters, status: 'approved' })}
                    style={{
                      flex: '1 1 45%',
                      borderColor: filters.status === 'approved' ? '#3b82f6' : 'hsl(var(--border))',
                      background: filters.status === 'approved' ? '#3b82f6' : 'hsl(var(--bg-card))',
                      color: filters.status === 'approved' ? 'white' : 'hsl(var(--text-muted))',
                    }}
                  >
                    Aprovados
                  </button>
                  <button
                    className={`tauze-tag-chip ${filters.status === 'rejected' ? 'active' : ''}`}
                    onClick={() => setFilters({ ...filters, status: 'rejected' })}
                    style={{
                      flex: '1 1 45%',
                      borderColor: filters.status === 'rejected' ? '#3b82f6' : 'hsl(var(--border))',
                      background: filters.status === 'rejected' ? '#3b82f6' : 'hsl(var(--bg-card))',
                      color: filters.status === 'rejected' ? 'white' : 'hsl(var(--text-muted))',
                    }}
                  >
                    Recusados
                  </button>
                ...
                <>
                  ...
                  <button
                    className={`tauze-tag-chip ${filters.status === 'active' ? 'active' : ''}`}
                    onClick={() => setFilters({ ...filters, status: 'active' })}
                    style={{
                      flex: '1 1 45%',
                      borderColor: filters.status === 'active' ? '#3b82f6' : 'hsl(var(--border))',
                      background: filters.status === 'active' ? '#3b82f6' : 'hsl(var(--bg-card))',
                      color: filters.status === 'active' ? 'white' : 'hsl(var(--text-muted))',
                    }}
                  >
                    Ativas
                  </button>
                  <button
                    className={`tauze-tag-chip ${filters.status === 'inactive' ? 'active' : ''}`}
                    onClick={() => setFilters({ ...filters, status: 'inactive' })}
                    style={{
                      flex: '1 1 45%',
                      borderColor: filters.status === 'inactive' ? '#3b82f6' : 'hsl(var(--border))',
                      background: filters.status === 'inactive' ? '#3b82f6' : 'hsl(var(--bg-card))',
                      color: filters.status === 'inactive' ? 'white' : 'hsl(var(--text-muted))',
                    }}
                  >
                    Inativas
                  </button>
          ...
          {isPendencies && (
            <>
              <div className="tauze-filter-section">
                <label className="tauze-filter-label">
                  Período da Solicitação <Calendar size={14} />
                </label>
