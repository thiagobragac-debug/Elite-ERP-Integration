...
export const CertificateSettingsTab: React.FC<CertificateSettingsTabProps> = ({
  searchTerm,
  triggerCreate,
}) => {
  ...
  const [isModalOpen, setIsModalOpen] = useState(() => {
    ...
    try {
      return sessionStorage.getItem(`${FORM_SESSION_KEY}_open`) === 'true';
    } catch {
      return false;
    }
  });
  ...
  const { data: certificados = [], isLoading: loadingCerts } = useQuery({
    ...
    queryFn: async () => {
      ...
      const { data, error } = await supabase.rpc('get_certificados_digitais', {
  ...
  const { data: empresas = [], isLoading: loadingEmpresas } = useQuery({
    ...
    queryFn: async () => {
      ...
      const { data, error } = await supabase
        .from('fazendas')
        .select('id, nome, ie_produtor, nirf')
  ...
  const upsertMutation = useMutation({
    ...
    onSuccess: () => {
      toast.success(
        editingCert ? 'Certificado atualizado com sucesso!' : 'Certificado importado com sucesso!'
      );
      clearDraft();
  ...
  const handleDelete = async (id: string) => {
    const isConfirmed = await confirm({
      title: 'Atenção',
      description:
        'Deseja realmente excluir este certificado? Ele não poderá mais assinar NF-e ou realizar buscas.',
  ...
  const filteredCerts = certificados.filter(
    (c: any) =>
      c.titular.toLowerCase().includes(searchTerm.toLowerCase()) || c.cnpj_cpf.includes(searchTerm)
  );
    ...
    },
    {
      ...
      accessor: (item: any) => {
        ...
        const diasRestantes = Math.floor(
          (validade.getTime() - agora.getTime()) / (1000 * 3600 * 24)
        );
        ...
        return (
          <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
            ...
            />
            <span
              style={{ fontSize: 13, fontWeight: 600, color: isVencido ? '#ef4444' : 'inherit' }}
            >
              ...
              >
                ...
                {empresas.map((emp) => (
                  <option key={emp.id} value={emp.id}>
                    {emp.nome} (IE: {emp.ie_produtor || 'N/A'})
                  </option>
                ))}
              </select>
            </div>
            ...
            <div className="tauze-field-group span-2" style={{ marginTop: '8px' }}>
              ...
              <div
                ...
                style={{
                  ...
                  textAlign: 'center',
                  cursor: 'pointer',
                  background: formData.pfx_base64
                    ? 'hsl(var(--brand) / 0.05)'
                    : 'hsl(var(--bg-main))',
              ...
              >
                ...
                <UploadCloud
                  ...
                  style={{
                    margin: '0 auto 16px',
                    color: formData.pfx_base64 ? 'hsl(var(--brand))' : 'hsl(var(--text-muted))',
                  }}
                />
                <h4
                  style={{
                    fontSize: '14px',
                    fontWeight: 700,
                    color: formData.pfx_base64 ? 'hsl(var(--brand))' : 'hsl(var(--text-main))',
                    marginBottom: '4px',
                  }}
                >
