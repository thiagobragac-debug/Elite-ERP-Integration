...
export const MarketIntelligenceDashboard: React.FC = () => {
  ...
  const [isAlertModalOpen, setIsAlertModalOpen] = usePersistentState(
    'MarketIntelligenceDashboard_isAlertModalOpen',
    false
  );
  ...
  const handleExport = (format: 'csv' | 'excel' | 'pdf') => {
    ...
    if (format === 'csv') {
      ...
      exportToPDF(exportData, filename, 'Histórico de Mercado');
    }
  };
  ...
  const currentDateLabel = liveDate
    ? `Dado de ${liveDate} · ao vivo`
    : stats.isEmpty
      ? ''
      : `Dado de ${stats.lastDate} · banco`;

  // Variação: do primeiro ao preço atual (live ou banco)
  const firstVal = data.length > 0 ? data[0].value : 0;
  const liveVariation =
    firstVal > 0 ? ((currentPrice - firstVal) / firstVal) * 100 : stats.variation;
  ...
  return (
    <div className="admin-intelligence-page animate-slide-up">
      ...
      <PriceAlertModal
        ...
        onSuccess={() =>
          toast.success(
            'Alerta de mercado cadastrado com sucesso! O Copilot vai monitorar os preços para você.'
          )
        }
      />
    </div>
