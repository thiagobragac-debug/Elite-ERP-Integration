...
vi.mock('../../hooks/useReportData', () => ({
  useReportData: vi.fn((reportType, options) => {
    return {
      data: [
        { id: '1', animais: { brinco: 'VACA-1' }, tipo_evento: 'IATF', data_evento: '2026-06-08', resultado: 'Aguardando', ecc: 3.5, touro: 'Touro Nelore', progressoGestacao: 0, status: 'completed' },
        { id: '2', animais: { brinco: 'VACA-2' }, tipo_evento: 'Parto', data_evento: '2026-06-08', resultado: 'Prenha', ecc: 2.5, touro: null, progressoGestacao: 95, status: 'completed' }
      ],
      stats: [
...
vi.mock('./components/BatchReproModal', () => ({
  BatchReproModal: ({ isOpen, onClose, onBatchSubmit }: any) => isOpen ? (
    <div data-testid="mock-batch-modal">
      <button onClick={() => onBatchSubmit([{ animal_id: 'vaca-1', tipo_evento: 'IATF' }])}>Lançar Lote Mock</button>
...
vi.mock('./components/ReproductionFilterModal', () => ({
  ReproductionFilterModal: ({ isOpen, onClose }: any) => isOpen ? (
...
vi.mock('../../components/Forms/ReproductionForm', () => ({
  ReproductionForm: ({ isOpen, onClose, onSubmit }: any) => isOpen ? (
