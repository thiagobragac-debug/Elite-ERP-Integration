...
import {
  ...
  AlertCircle, ChevronRight, FileSpreadsheet, Database
} from 'lucide-react';
...
export const AuditLog: React.FC = () => {
  ...
  const { data: logs, stats, totalCount, loading, error } = useReportData('audit-logs', {
    page,
    pageSize,
    filters: {
      ...filterValues,
  ...
  return (
    <div className="audit-log-page">
        ...
        {loading ? (
          <div className="premium-card audit-card" style={{ padding: '8px' }}>
            {Array(6).fill(0).map((_, i) => (
              <div key={i} className="audit-entry" style={{ display: 'flex', gap: '12px', alignItems: 'center', padding: '10px 14px' }}>
                <div className="skeleton-base" style={{ width: '34px', height: '34px', borderRadius: '10px', flexShrink: 0 }} />
      ...
      >
        {selectedLog && (
          <div style={{ gridColumn: '1 / -1', display: 'flex', flexDirection: 'column', gap: '16px', width: '100%' }}>
              ...
              <>
                ...
                {selectedLog.action === 'DELETE' && (
                  <div style={{
                    ...
                    <AlertCircle size={14} />
                    <span>Este registro foi excluído do banco de dados e não está mais disponível para visualização.</span>
                  </div>
                )}
                ...
                {selectedLog.action !== 'DELETE' && !dossierRoute && (
                  <div style={{
                    ...
                    <AlertCircle size={14} />
                    <span>Não há uma tela direta de gerenciamento associada para a tabela "{selectedLog.table_name}".</span>
                  </div>
                )}
                ...
                {(() => {
                  ...
                  const isFinancial = selectedLog.table_name === 'contas_pagar' || selectedLog.table_name === 'contas_receber';
                    ...
                    return (
                      <div className="dossier-slip">
                        <div className="dossier-slip-header">
                          ...
                          <span className={`status-pill ${status === 'PAGO' ? 'active' : 'warning'}`}>{status}</span>
                        </div>
                        <div className="dossier-slip-amount">
                          <span className="currency">R$</span>
                          <span className="value">{Number(amount).toLocaleString('pt-BR', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</span>
                        </div>
                        ...
                        <div className="dossier-slip-footer">
                          ...
                          {date && (
                            <div className="dossier-slip-meta-item">
                              ...
                              <span className="meta-val">{new Date(date).toLocaleDateString('pt-BR')}</span>
                            </div>
                          )}
                        </div>
                      </div>
                  ...
                  if (isAnimal) {
                    ...
                    return (
                      <div className="dossier-slip">
                        ...
                        <div className="dossier-slip-desc">{raca} • {data.sexo === 'M' ? 'Macho' : 'Fêmea'}</div>
                        <div className="dossier-slip-footer">
                          {peso && (
                            <div className="dossier-slip-meta-item">
                  ...
                  return (
                    <div className="dossier-slip">
                      <div className="dossier-slip-header">
                        <span className="dossier-slip-category">Tabela: {selectedLog.table_name}</span>
                        {data.status && <span className={`status-pill active`}>{data.status}</span>}
                      </div>
                      <div className="dossier-slip-amount">
                        <span className="value" style={{ fontSize: '1.6rem', padding: '10px 0' }}>{mainTitle}</span>
                      </div>
                ...
                {(() => {
                  ...
                  const allKeys = Array.from(new Set([...Object.keys(oldData), ...Object.keys(newData)]))
                    .filter(k => k !== 'tenant_id' && k !== 'fazenda_id' && k !== 'id' && k !== 'created_at' && k !== 'updated_at' && k !== 'fazendas_vinculadas')
                    .sort();
                  ...
                  return (
                    <>
                      <div className="diff-grid-header">
                        ...
                        >
                          {showOnlyChanges ? "Mostrar Todos os Campos" : "Mostrar Apenas Alterações"}
                        </button>
                      </div>
                      ...
                      <table className="diff-table">
                        ...
                        <tbody>
                          {displayedKeys.length === 0 ? (
                            <tr>
                              <td colSpan={3} style={{ textAlign: 'center', color: 'hsl(var(--text-muted))', padding: '20px' }}>
                                ...
                                <tr key={key} className={isChanged ? "diff-row-changed" : ""}>
                                  ...
                                  <td>
                                    {isChanged && oldVal !== undefined ? (
                                      <span className="diff-badge old">{formatVal(key, oldVal)}</span>
                                    ) : (
                                      <span style={{ color: 'hsl(var(--text-muted))' }}>{formatVal(key, oldVal)}</span>
                                    )}
                                  </td>
                                  <td>
                                    {isChanged ? (
                                      <span className="diff-badge new" style={{ fontWeight: 800 }}>{formatVal(key, newVal)}</span>
                                    ) : (
                                      <span style={{ color: 'hsl(var(--text-muted))' }}>{formatVal(key, newVal)}</span>
                                    )}
                                  </td>
                                </tr>
                              );
            ...
            <div style={{ marginTop: '12px', paddingTop: '12px', borderTop: '1px solid hsl(var(--border))', width: '100%' }}>
      ...
      >
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(280px, 1fr))', gap: '16px', width: '100%' }}>
          {formInitialData && Object.keys(formInitialData)
            .filter(key => key !== 'id' && key !== 'tenant_id' && key !== 'created_at' && key !== 'updated_at' && typeof formInitialData[key] !== 'object')
            .map(key => {
                ...
                return (
                  <div key={key} className="tauze-input-group" style={{ display: 'flex', alignItems: 'center', gap: '10px', gridColumn: 'span 1' }}>
                    <input
                      ...
                      id={`dyn-${key}`}
                      checked={val}
                      onChange={(e) => setFormInitialData({ ...formInitialData, [key]: e.target.checked })}
                      style={{ width: '18px', height: '18px', cursor: 'pointer' }}
                    />
                    <label htmlFor={`dyn-${key}`} style={{ margin: 0, cursor: 'pointer', fontWeight: 600, fontSize: '13px' }}>
                      {label}
                    </label>
                  </div>
                );
              ...
              return (
                <div key={key} className="tauze-input-group">
                  ...
                  <input
                    ...
                    value={val !== null && val !== undefined ? (isDate ? val.toString().substring(0, 10) : val.toString()) : ''}
                    onChange={(e) => {
          ...
          {formInitialData && (
            <div style={{
              ...
              {formInitialData.id && <div><strong>ID do Registro:</strong> {formInitialData.id}</div>}
              {formInitialData.created_at && <div><strong>Criado em:</strong> {new Date(formInitialData.created_at).toLocaleString('pt-BR')}</div>}
              {formInitialData.updated_at && <div><strong>Atualizado em:</strong> {new Date(formInitialData.updated_at).toLocaleString('pt-BR')}</div>}
            </div>
          )}
        </div>
      </SidePanel>
