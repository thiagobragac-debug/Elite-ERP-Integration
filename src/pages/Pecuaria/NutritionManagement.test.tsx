...
vi.mock('../../hooks/useReportData', () => ({
  useReportData: vi.fn((reportType, options) => {
    return {
      data: [
        { id: '1', nome: 'Dieta Engorda', tipo: 'RACAO', custo_por_kg: 2.5, percMS: 80, custoMS: 3.12, status: 'active', ingredientes: ['Milho', 'Soja'] },
        { id: '2', nome: 'Milho Moído', tipo: 'MATERIA_PRIMA', custo_por_kg: 1.2, percMS: 88, custoMS: 1.36, status: 'active', ingredientes: [] }
      ],
      stats: [
...
vi.mock('../../components/Forms/DietForm', () => ({
  DietForm: ({ isOpen, onClose, onSubmit }: any) => isOpen ? (
    <div data-testid="mock-diet-form">
      <button onClick={() => onSubmit({ nome: 'Nova Dieta Mock', custo_por_kg: 1 })}>Salvar Mock</button>
...
vi.mock('./components/NutritionSimulatorModal', () => ({
  NutritionSimulatorModal: ({ isOpen, onClose }: any) => isOpen ? (
...
vi.mock('./components/NutritionFilterModal', () => ({
  NutritionFilterModal: ({ isOpen, onClose }: any) => isOpen ? (
