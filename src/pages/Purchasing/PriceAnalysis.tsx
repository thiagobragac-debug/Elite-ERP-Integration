...
export const PriceAnalysis: React.FC = () => {
  ...
  const fetchPriceData = async () => {
    ...
    if (data) {
      ...
      const totalSaving = analysis.reduce(
        (acc: number, p: any) => acc + Math.max(0, p.avgPrice - p.minPrice),
        0
      );
      ...
      setStats([
        {
          ...
          value:
            totalSaving > 0
              ? totalSaving.toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' })
              : '---',
          icon: DollarSign,
          color: '#10b981',
        ...
        },
        {
          ...
          progress:
            analysis.length > 0
              ? (analysis.filter((p: any) => p.variation === 'up').length / analysis.length) * 100
              : 0,
  ...
  const tableColumns = [
    ...
    },
    {
      ...
      accessor: (item: any) => (
        <div style={{ display: 'flex', justifyContent: 'center' }}>
          <span
            className={`status-pill ${item.variation === 'up' ? 'stopped' : item.variation === 'down' ? 'active' : 'warning'}`}
      ...
      <header className="page-header">
        <div className="header-brand-group">
          ...
          <p className="page-subtitle">
            Monitoramento de variações de custo, tendências de mercado e inteligência de compra
