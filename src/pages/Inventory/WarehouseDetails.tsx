  ...
  if (!warehouse && !loading) {
    return (
      <div style={{ padding: '40px', textAlign: 'center' }}>
        ...
        <button className="primary-btn" onClick={() => navigate('/estoque/deposito')} style={{ marginTop: '20px' }}>Voltar</button>
      </div>
    );
  }
      ...
      accessor: (item: any) => (
        <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '2px' }}>
            ...
            <span style={{ fontSize: '11px', fontWeight: 700, color: '#94a3b8', textTransform: 'uppercase' }}>{item.categoria || 'Geral'}</span>
          </div>
        </div>
      )
    },
    {
      ...
      accessor: (item: any) => (
        <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
          <span style={{ fontSize: '16px', fontWeight: 900, color: '#0f172a' }}>{Number(item.saldo).toLocaleString('pt-BR')}</span>
          <span style={{ fontSize: '12px', fontWeight: 700, color: '#64748b' }}>{item.unidade}</span>
        </div>
      )
    },
    {
  ...
  const historyColumns = [
      ...
      accessor: (item: any) => {
        ...
        return (
          <div style={{ display: 'flex', flexDirection: 'column' }}>
            <span style={{ fontWeight: 700, color: '#1e293b' }}>{d.toLocaleDateString('pt-BR')}</span>
            <span style={{ fontSize: '11px', color: '#94a3b8', fontWeight: 600 }}>{d.toLocaleTimeString('pt-BR', { hour: '2-digit', minute: '2-digit' })}</span>
          </div>
        );
      }
    },
    {
      ...
      accessor: (item: any) => {
        ...
        return (
          <span className={`status-pill ${isEntry ? 'active' : 'danger'}`} style={{ display: 'inline-flex', gap: '4px', alignItems: 'center' }}>
    ...
    },
    {
      header: 'Insumo',
      accessor: (item: any) => (
        <span style={{ fontWeight: 700, color: '#334155' }}>{item.produtos?.nome || 'Item Desconhecido'}</span>
      )
    },
    {
      header: 'Quantidade',
      accessor: (item: any) => {
        ...
        return (
          <span style={{ fontWeight: 800, color: isEntry ? '#10b981' : '#ef4444' }}>
            {isEntry ? '+' : '-'}{Number(item.quantidade).toLocaleString('pt-BR')} {item.produtos?.unidade_medida || 'un'}
          </span>
        );
      }
    },
    ...
    <div className="inventory-page animate-slide-up" style={{ paddingBottom: '40px' }}>
      ...
      <div className="next-gen-kpi-grid" style={{ marginBottom: '32px' }}>
        ...
        <TauzeStatCard 
          ...
          icon={Package} 
          color="#f59e0b"
          progress={warehouse?.capacidade_maxima > 0 ? (totalQuantity / warehouse.capacidade_maxima) * 100 : 0}
          change={warehouse?.capacidade_maxima > 0 ? `${Math.round((totalQuantity / warehouse.capacidade_maxima) * 100)}% de Ocupação` : 'Capacidade N/A'}
      ...
      <div className="details-content" style={{ background: 'white', borderRadius: '24px', border: '1px solid hsl(var(--border))', padding: '24px' }}>
        {activeTab === 'stock' && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '24px' }}>
              <div>
                <h3 style={{ fontSize: '18px', fontWeight: 800, color: '#0f172a' }}>Saldo por Insumo</h3>
                <p style={{ fontSize: '13px', color: '#64748b' }}>Posição consolidada das entradas e saídas neste local.</p>
              </div>
            </div>
            <ModernTable 
              data={stockItems}
        ...
        {activeTab === 'history' && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '24px' }}>
              <div>
                <h3 style={{ fontSize: '18px', fontWeight: 800, color: '#0f172a' }}>Últimas Movimentações</h3>
                <p style={{ fontSize: '13px', color: '#64748b' }}>Histórico de transações exclusivas deste depósito.</p>
              </div>
            </div>
            <ModernTable 
              data={history}
