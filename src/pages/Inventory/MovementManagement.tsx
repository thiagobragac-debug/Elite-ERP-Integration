...
export const MovementManagement: React.FC = () => {
  ...
  const [isHistoryModalOpen, setIsHistoryModalOpen] = usePersistentState(
    'MovementManagement_isHistoryModalOpen',
    false
  );
  ...
  const [showAdvancedFilters, setShowAdvancedFilters] = usePersistentState(
    'MovementManagement_showAdvancedFilters',
    false
  );
  ...
  const handleExport = async (format: 'csv' | 'excel' | 'pdf') => {
    ...
    const exportData = filteredData.map((item) => ({
      ...
      Tipo:
        item.tipo === 'ENTRADA' || item.tipo === 'in'
          ? 'Entrada'
          : item.tipo === 'TRANSFERENCIA' || item.tipo === 'transfer'
            ? 'Transferência'
            : 'Saída',
    ...
    if (format === 'csv') {
      exportToCSV(exportData, 'log_movimentacoes');
    } else if (format === 'excel') {
      exportToExcel(exportData, 'log_movimentacoes');
    } else if (format === 'pdf') {
      exportToPDF(exportData, 'log_movimentacoes', 'Relatório de Movimentação de Estoque');
    }
  };
  ...
  const handleViewDetails = (move: any) => {
    ...
    setHistoryItems([
      {
        id: '1',
        date: move.data_movimentacao,
        title:
          move.tipo === 'ENTRADA' || move.tipo === 'in'
            ? 'Entrada de Mercadoria'
            : 'Saída de Mercadoria',
        subtitle: `Produto: ${move.produtos?.nome || 'Item'}`,
        value: `${move.quantidade} ${move.produtos?.unidade || ''}`,
        status: move.tipo === 'ENTRADA' || move.tipo === 'in' ? 'success' : 'error',
      },
      {
        id: '2',
        date: move.data_movimentacao,
        title: 'Documento de Origem',
        subtitle: move.origem_destino || 'N/A',
        value: 'Vinculado',
        status: 'info',
      },
      {
    ...
    },
    {
      ...
      accessor: (item: any) => (
        <div style={{ display: 'flex', justifyContent: 'center' }}>
          <span
            className={`status-pill ${item.tipo === 'ENTRADA' || item.tipo === 'in' ? 'success' : item.tipo === 'TRANSFERENCIA' || item.tipo === 'transfer' ? 'warning' : 'danger'}`}
    ...
    },
    {
      ...
      accessor: (item: any) => (
        <div style={{ display: 'flex', flexDirection: 'column', gap: '2px', textAlign: 'left' }}>
          ...
          {item.data_validade ? (
            <span
              className={`flex items-center gap-1 text-[10px] font-bold ${new Date(item.data_validade) < new Date() ? 'text-red-500' : 'text-amber-500'}`}
            >
              <Calendar size={10} /> Val: {new Date(item.data_validade).toLocaleDateString()}
            </span>
          ) : (
            <span style={{ fontSize: '9px', fontWeight: 700, color: '#94a3b8' }}>Sem Validade</span>
          )}
        </div>
      ),
      align: 'left' as const,
      ...
      accessor: (item: any) => (
        <div style={{ display: 'flex', justifyContent: 'center' }}>
          <span style={{ fontSize: '12px', fontWeight: 900, color: '#059669' }}>
            R${' '}
            {(Number(item.quantidade) * Number(item.custo_unitario || 0)).toLocaleString('pt-BR')}
          </span>
        </div>
      ),
      align: 'center' as const,
    },
    {
      ...
      accessor: (item: any) => (
        ...
        >
          ...
          >
            <Calendar size={12} />{' '}
            {item.data_movimentacao ? new Date(item.data_movimentacao).toLocaleDateString() : 'N/A'}
          </span>
          <span
            style={{
              fontSize: '9px',
      ...
      <header className="page-header">
        <div className="header-brand-group">
          ...
          <p className="page-subtitle">
            Rastreabilidade total de entradas, saídas e transferências de insumos em tempo real.
          </p>
        </div>
          ...
          <div className="export-dropdown-container">
            ...
            <div id="export-menu-movements" className="export-menu">
              <button
                onClick={() => {
                  handleExport('csv');
                  document.getElementById('export-menu-movements')?.classList.remove('active');
                }}
              >
                Excel (.CSV)
              </button>
        ...
        <ModernTable
          emptyState={
            ...
            !filterValues.dateEnd ? (
              <EmptyState
                title={
                  activeTab === 'LOG' ? 'Nenhum movimento registrado' : 'Nenhuma análise disponível'
                }
                description={
                  activeTab === 'LOG'
                    ? 'O log de movimentações está vazio no momento.'
                    : 'Não há dados suficientes para a análise de fluxo.'
                }
          ...
          actions={(item) => (
            <div className="modern-actions">
              ...
              {(!item.origem_destino ||
                (!item.origem_destino.includes('[REF:') &&
