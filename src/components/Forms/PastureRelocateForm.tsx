        ...
        {adding > 0 && (
          <div
            style={{
              ...
              top: 0,
              height: '100%',
              width: `${Math.min((adding / (cap.max || 1)) * 100, 100 - beforePct)}%`,
              background: color,
              opacity: 0.7,
      ...
      {pct > 100 && (
        ...
        >
          ...
          <span style={{ fontSize: '11px', fontWeight: 700, color: '#ef4444' }}>
            Atenção: transferência ultrapassará a capacidade de suporte do pasto
          </span>
        </div>
      )}
    </div>
...
export const PastureRelocateForm: React.FC<PastureRelocateFormProps> = ({
  ...
  useEffect(() => {
    if (targetPastureId) {
      fetchTargetCapacity(targetPastureId);
    } else {
      setDestCap(null);
    }
  }, [targetPastureId]);
  ...
  const fetchPastures = async () => {
    ...
    if (error) {
      console.error('[PastureRelocateForm] fetchPastures error:', error);
    }
    if (data) {
      setPastures(data);
    }
  ...
  const sourceName =
    sourcePastureName || pastures.find((p) => p.id === sourcePastureId)?.nome || '';
  const targetName =
    targetPastureName || pastures.find((p) => p.id === targetPastureId)?.nome || '';
    ...
    try {
      ...
      if (!error) {
        ...
        onSubmit({
          ...
          source: sourcePastureId,
          target: targetPastureId,
        });
        onClose();
      }
    } catch (err) {
  ...
  const confirmOverlay = showConfirm
    ? ReactDOM.createPortal(
        (() => {
          ...
          const afterUaHa =
            destCap && destCap.area > 0 ? (afterUa / destCap.area).toFixed(2) : null;
            ...
            >
              ...
              >
                ...
                >
                  ...
                  <div>
                    <h3 style={{ margin: 0, fontSize: '16px', fontWeight: 800 }}>
                      Confirmar Remanejamento de Pasto
                    </h3>
                ...
                >
                  ...
                  <Row
                    label="Animais"
                    value={`${selectedAnimals.length} cab. (${selectedUaTotal.toFixed(1)} UA)`}
                  />
                  ...
                  {afterUaHa && (
                    <Row
                      label="Pressão de pastejo pós-mov."
                      value={`${afterUaHa} UA/ha (${afterPct}% cap.)`}
                      color={
                        afterPct && afterPct > 100
                          ? '#ef4444'
                          : afterPct && afterPct > 85
                            ? '#f59e0b'
                            : '#10b981'
                      }
                    />
          ...
          <div className="tauze-input-grid grid-col-4">
            <div className="tauze-field-group">
              <PastureSearch
                ...
                onChange={(id, name) => {
                  setSourcePastureId(id);
                  setSourcePastureName(name);
                }}
                placeholder="Buscar pasto de origem..."
                label={
                  <>
                    <Trees size={14} /> Pasto de Origem
                  </>
                }
              />
            </div>
            ...
            <div className="tauze-field-group">
              <PastureSearch
                ...
                onChange={(id, name) => {
                  setTargetPastureId(id);
                  setTargetPastureName(name);
                }}
                placeholder="Buscar pasto de destino..."
                label={
                  <>
                    <Trees size={14} /> Pasto de Destino
                  </>
                }
                ...
                if (target && (target.status === 'resting' || target.status === 'renovation')) {
                  return (
                    ...
                    >
                      ...
                      {target.status === 'resting' ? 'Descanso' : 'Reforma'}. A transferência pode
                      prejudicar o pasto.
                    </div>
                  );
                }
            ...
            <div className="tauze-animal-picker">
              {loading ? (
                ...
                <div className="picker-empty">
                  Selecione um pasto de origem para ver os animais.
                </div>
              ) : animals.length === 0 ? (
