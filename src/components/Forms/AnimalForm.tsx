...
const formatRFID = (value: string) => {
  ...
  if (limited.length > 11) {
    formatted += ` ${limited.substring(11, 15)}`;
  }
  return formatted;
};
...
export const AnimalForm: React.FC<AnimalFormProps> = ({
    ...
    if (initialData) {
      setFormData({
        ...
        valor_venda: initialData.valor_venda
          ? initialData.valor_venda.toString().replace(/[^\d.-]/g, '')
          : '',
  ...
  const handleIdadeChange = (meses: string) => {
    setFormData((prev) => {
      if (!meses) {
        return { ...prev, idade_meses: '', data_nascimento: '' };
      }
  ...
  const handleDataNascimentoChange = (dataStr: string) => {
    setFormData((prev) => {
      if (!dataStr) {
        return { ...prev, data_nascimento: '', idade_meses: '' };
      }
  ...
  React.useEffect(() => {
    ...
    if (formData.sexo === 'M') {
      if (months <= 12) {
        suggestedCat = 'Bezerro';
      } else if (months <= 24) {
        suggestedCat = 'Garrote';
      } else if (months <= 36) {
        suggestedCat = 'Boi Magro';
      } else {
        suggestedCat = 'Boi Gordo';
      }
    } else if (months <= 12) {
      suggestedCat = 'Bezerra';
    } else if (months <= 24) {
      suggestedCat = 'Novilha';
    } else {
      suggestedCat = 'Vaca';
    }
    ...
    >
            ...
            >
              {formData.brinco_eletronico.length <= 3 && (
                <span
                  style={{
                    color:
                      formData.brinco_eletronico.length > 0 ? 'inherit' : 'hsl(var(--text-muted))',
      ...
      <section className="tauze-form-section">
        ...
        <div className="tauze-input-grid grid-col-3">
          <div className="tauze-field-group">
            <label className="tauze-label">
              <Info size={14} /> Peso de Entrada (kg)
            </label>
            <input
              className="tauze-input"
              type="number"
          ...
          <div className="tauze-field-group">
            <label className="tauze-label">
              ...
              {formData.idade_meses ? (
                <span style={{ fontSize: '10px', color: 'var(--brand)' }}>(Auto-Sugerido)</span>
              ) : (
                ''
              )}
            </label>
