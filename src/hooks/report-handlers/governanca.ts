 ...
 */
export const auditLogs: ReportHandler = async (
  tenantId,
  fazendaId,
  page = 1,
  pageSize = 25,
  ...
  const columns = [
    ...
    { header: 'Ação', accessor: 'action' },
    {
      header: 'Data / Hora',
      accessor: (row: any) =>
        row.timestamp ? new Date(row.timestamp).toLocaleString('pt-BR') : 'N/A',
    },
  ...
  try {
    ...
    let query = supabase
      .from('audit_logs')
      .select(
        'id, action, entity, entity_id, description, created_at, user_id, old_data, new_data',
        { count: 'exact' }
      )
    ...
    if (filters.module && filters.module !== 'ALL') {
      query = query.eq('entity', filters.module);
    }
    ...
    if (filters.search) {
      query = query.or(`description.ilike.%${filters.search}%,entity.ilike.%${filters.search}%`);
    }
      ...
      stats: [
        {
          ...
          subtitle: _lastLogDate
            ? `Último evento em ${_lastLogDate}`
        ...
        },
        {
          ...
          subtitle:
            deletes > 0 && _lastLogDate
              ? `Último alerta em ${_lastLogDate}`
              : `Status em ${todayBR()}`,
          sparkline: buildSparkline(
            logs.filter((l: any) => l.action === 'DELETE'),
            'created_at',
            null
          ),
 ...
 */
export const perfisUsuario: ReportHandler = async (
  tenantId,
  fazendaId,
  page = 1,
  pageSize = 25
...
export const adminOverview: ReportHandler = async (tenantId, fazendaId) => {
  const mockData = {
    ...
    columns: [
      ...
      { header: 'Descrição', accessor: 'description' },
      {
        header: 'Data / Hora',
        accessor: (row: any) =>
          row.created_at ? new Date(row.created_at).toLocaleString('pt-BR') : 'N/A',
      },
    ],
    stats: [],
    totalCount: 2,
  ...
  try {
    const [userRes, logsRes] = await Promise.all([
      ...
      supabase
        .from('audit_logs')
        .select('id, action, entity, description, created_at', { count: 'exact' })
        .eq('tenant_id', tenantId)
        .order('created_at', { ascending: false })
        .limit(20),
    ]);
    ...
    const _lastAdminLogDate =
      logsRes.data && logsRes.data.length > 0 ? fmtDateBR(logsRes.data[0].created_at) : null;
    const criticalEvents = (logsRes.data || []).filter(
      (l: any) => l.action === 'DELETE' || l.action === 'SECURITY_ALERT'
    ).length;
      ...
      stats: [
        {
          ...
          subtitle: _lastAdminLogDate
            ? `Calculado em ${_lastAdminLogDate}`
        ...
        },
        {
          ...
          subtitle:
            criticalEvents > 0 && _lastAdminLogDate
              ? `Último alerta em ${_lastAdminLogDate}`
              : `Status em ${todayBR()}`,
          sparkline: buildSparkline(
            (logsRes.data || []).filter(
              (l: any) => l.action === 'DELETE' || l.action === 'SECURITY_ALERT'
