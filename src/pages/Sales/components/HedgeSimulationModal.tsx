...
export const HedgeSimulationModal: React.FC<HedgeSimulationModalProps> = ({ isOpen, onClose }) => {
  const [params, setParams] = useState({
    ...
    exchangeRate: 5.20,
    quantity: 1000,
    currentPrice: 120.00, // Preço Base
    targetPrice: 130.00,  // Strike da Opção / Trava do Futuro
    costOfHedge: 2.50,    // Prêmio da Put ou Corretagem
  });
  ...
  const calculateSimulation = () => {
    ...
    const variations = [
      { label: 'Queda Drástica', pct: -0.20 },
      { label: 'Queda Leve', pct: -0.05 },
      { label: 'Estabilidade', pct: 0.00 },
      { label: 'Alta Leve', pct: 0.05 },
      { label: 'Alta Expressiva', pct: 0.20 },
    ];
    ...
    const scenarios = variations.map(v => {
      ...
      if (params.instrument === 'put') {
        ...
        if (simulatedPriceBase < params.targetPrice) {
          financialGainBRL = ((params.targetPrice - simulatedPriceBase) * params.quantity) * params.exchangeRate;
        }
    ...
    if (downScenarios.length > 0) {
      ...
      const avgNet = downScenarios.reduce((acc, s) => acc + s.netResultBRL, 0) / downScenarios.length;
  ...
  return (
    ...
    >
      <div style={{ display: 'grid', gridTemplateColumns: '380px 1fr', gap: '32px', gridColumn: 'span 2' }}>
        
        {/* LADO ESQUERDO: PARÂMETROS DA OPERAÇÃO */}
        <div style={{ display: 'flex', flexDirection: 'column', gap: '24px', background: 'hsl(var(--bg-card))', padding: '24px', borderRadius: '16px', border: '1px solid hsl(var(--border))' }}>
          
          <div style={{ fontSize: '13px', fontWeight: 800, color: 'hsl(var(--text-main))', display: 'flex', alignItems: 'center', gap: '8px', borderBottom: '1px solid hsl(var(--border))', paddingBottom: '12px' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px' }}>
            ...
            <div className="tauze-input-field">
              <label style={{ display: 'flex', alignItems: 'center', gap: '6px', fontSize: '11px', fontWeight: 700, color: 'hsl(var(--success))' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px' }}>
            <div className="tauze-input-field">
              <label style={{ display: 'flex', alignItems: 'center', gap: '6px', fontSize: '11px', fontWeight: 700, color: 'hsl(var(--danger))' }}>
          ...
          <div style={{ padding: '16px', borderRadius: '12px', background: 'hsl(var(--brand)/0.05)', border: '1px solid hsl(var(--brand)/0.2)', marginTop: 'auto' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '6px', color: 'hsl(var(--brand))', fontSize: '11px', fontWeight: 800, marginBottom: '8px' }}>
              <Info size={14} /> DICA TAUZE
            </div>
            <p style={{ margin: 0, fontSize: '11px', lineHeight: '1.5', color: 'hsl(var(--text-muted))', fontWeight: 500 }}>
              Use a opção <strong>"Opção de Venda (Put)"</strong> para proteção contra quedas sem limitar ganhos de alta (seguro de safra). Use <strong>"Mercado Futuro"</strong> para travar o preço estático.
            </p>
          </div>
        </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '20px' }}>
            <div style={{ padding: '24px', borderRadius: '16px', background: 'hsl(var(--bg-main))', border: '1px dashed hsl(var(--border))', display: 'flex', flexDirection: 'column', gap: '4px' }}>
              <span style={{ fontSize: '11px', fontWeight: 900, color: 'hsl(var(--text-muted))', textTransform: 'uppercase' }}>Valor Físico (S/ Hedge)</span>
              <div style={{ fontSize: '24px', fontWeight: 900, color: 'hsl(var(--text-main))' }}>
                {results.exposureBRL.toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' })}
              </div>
            </div>
            <div style={{ padding: '24px', borderRadius: '16px', background: 'linear-gradient(135deg, hsl(var(--success)) 0%, hsl(var(--success-dark, 142 71% 25%)) 100%)', display: 'flex', flexDirection: 'column', gap: '4px', color: 'white', boxShadow: '0 10px 25px -5px hsl(var(--success)/0.3)' }}>
              <span style={{ fontSize: '11px', fontWeight: 900, opacity: 0.9, textTransform: 'uppercase' }}>Piso Protegido (C/ Hedge)</span>
              <div style={{ fontSize: '24px', fontWeight: 900 }}>
                {results.protectedValueBRL.toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' })}
              </div>
              <span style={{ fontSize: '10px', opacity: 0.8 }}>Custo total: {results.totalCostBRL.toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' })}</span>
            </div>
          </div>

          {/* Análise de Stress: Hedge Accounting */}
          <div style={{ flex: 1, padding: '24px', borderRadius: '16px', background: 'hsl(var(--bg-main)/0.2)', border: '1px solid hsl(var(--border))' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '24px' }}>
              <div>
                <span style={{ fontSize: '14px', fontWeight: 800, display: 'block' }}>Análise de Stress (Hedge Accounting)</span>
                <span style={{ fontSize: '11px', color: 'hsl(var(--text-muted))' }}>Composição: Físico + Financeiro</span>
              </div>
              <div style={{ padding: '6px 12px', borderRadius: '8px', background: 'hsl(var(--success)/0.1)', color: 'hsl(var(--success))', fontSize: '11px', fontWeight: 900 }}>
                ...
                return (
                  <div key={idx} style={{ 
                    ...
                    <div style={{ width: '130px' }}>
                      <div style={{ fontSize: '13px', fontWeight: 800, color: isPriceDown ? 'hsl(var(--danger))' : 'hsl(var(--text-main))' }}>
                        {scenario.label}
                      </div>
                      <div style={{ fontSize: '11px', color: 'hsl(var(--text-muted))', fontWeight: 600 }}>Cotação a {scenario.simulatedPriceBase.toFixed(2)}</div>
                    </div>

                    {/* Matemática do Hedge Accounting */}
                    <div style={{ flex: 1, display: 'flex', gap: '16px', alignItems: 'center', borderLeft: '1px dashed hsl(var(--border))', paddingLeft: '20px' }}>
                      
                      {/* Venda Física */}
                      <div>
                        <div style={{ fontSize: '10px', fontWeight: 700, color: 'hsl(var(--text-muted))', textTransform: 'uppercase' }}>Físico</div>
                        <div style={{ fontSize: '12px', fontWeight: 700, color: 'hsl(var(--text-main))' }}>
                          {scenario.physicalResultBRL.toLocaleString('pt-BR', { style: 'currency', currency: 'BRL', maximumFractionDigits: 0 })}
                        </div>
                      </div>
                      ...
                      <div>
                        <div style={{ fontSize: '10px', fontWeight: 700, color: 'hsl(var(--text-muted))', textTransform: 'uppercase' }}>Bolsa/Opção</div>
                        <div style={{ fontSize: '12px', fontWeight: 800, color: scenario.financialGainBRL > 0 ? 'hsl(var(--success))' : 'hsl(var(--danger))' }}>
                          {scenario.financialGainBRL.toLocaleString('pt-BR', { style: 'currency', currency: 'BRL', maximumFractionDigits: 0 })}
                        </div>
                      </div>

                      <ArrowRight size={14} color="hsl(var(--text-muted))" style={{ marginLeft: 'auto' }} />

                      {/* Resultado Líquido Final */}
                      <div style={{ textAlign: 'right' }}>
                        <div style={{ fontSize: '10px', fontWeight: 800, color: 'hsl(var(--brand))', textTransform: 'uppercase', marginBottom: '2px' }}>
                          Caixa Líquido
                        </div>
                        <div style={{ fontSize: '15px', fontWeight: 900, color: 'hsl(var(--text-main))' }}>
                          {scenario.netResultBRL.toLocaleString('pt-BR', { style: 'currency', currency: 'BRL', maximumFractionDigits: 0 })}
                        </div>
                      </div>
            ...
            <div style={{ marginTop: '20px', fontSize: '11px', color: 'hsl(var(--text-muted))', textAlign: 'center' }}>
              * Os cálculos incluem o desconto do Custo da Operação (Prêmio/Corretagem) de {results.totalCostBRL.toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' })}.
            </div>
