...
export const HealthProtocolsModal: React.FC<HealthProtocolsModalProps> = ({
  isOpen,
  onClose,
  onApply,
}) => {
    ...
    try {
      ...
      if (isEditingId && !isDefault) {
        ...
        const { data, error } = await supabase
          ...
          .eq('id', isEditingId).eq('tenant_id', activeTenantId)
          .select()
          .single();
  ...
  const handleConfirmApplication = () => {
    if (!targetId) {
      return toast.error('Selecione um animal ou lote');
    }
  ...
  const smartSchedule = useMemo(() => {
    ...
    return selectedProtocol.steps.map((step: any) => {
      ...
      return {
        ...step,
        scheduledDateStr: scheduledDate.toLocaleDateString('pt-BR', {
          day: '2-digit',
          month: '2-digit',
          year: 'numeric',
        }),
  ...
  return (
    <SidePanel
      ...
      icon={ShieldCheck}
      submitLabel={
        isApplying ? 'Confirmar Aplicação' : isCreating ? 'Salvar Protocolo' : 'Aplicar Protocolo'
      }
            ...
            {protocols.map((p) => {
              ...
              return (
                <div key={p.id} style={{ position: 'relative' }} className="group">
                  ...
                  >
                    <div
                      style={{ color: isActive ? 'hsl(var(--brand))' : 'hsl(var(--text-muted))' }}
                    >
            ...
            {isApplying ? (
              ...
              >
                <div style={{ marginBottom: '24px' }}>
                  ...
                  <p style={{ fontSize: '13px', color: 'hsl(var(--text-muted))' }}>
                    Protocolo: <strong>{selectedProtocol?.name}</strong>
                  </p>
                </div>
                  ...
                  >
                    <button
                      ...
                      style={{
                        ...
                        fontWeight: 800,
                        borderRadius: '8px',
                        background: targetType === 'ANIMAL' ? 'white' : 'transparent',
                        border: 'none',
                        cursor: 'pointer',
                      }}
                    >
                    ...
                    <button
                      ...
                      style={{
                        ...
                        fontWeight: 800,
                        borderRadius: '8px',
                        background: targetType === 'LOTE' ? 'white' : 'transparent',
                        border: 'none',
                        cursor: 'pointer',
                      }}
                    >
                            ...
                            >
                              <div
                                style={{
                                  ...
                                  alignItems: 'center',
                                  justifyContent: 'center',
                                  color: step.isToday ? 'white' : 'transparent',
                                  marginTop: '4px',
                                }}
                              >
                              ...
                              <div
                                style={{
                                  flex: 1,
                                  padding: '12px 16px',
                                  background: step.isToday ? 'hsl(var(--brand)/0.05)' : 'white',
                                  border: `1px solid ${step.isToday ? 'hsl(var(--brand)/0.3)' : 'hsl(var(--border))'}`,
                                  borderRadius: '12px',
                                }}
                              >
                                <div
                  ...
                  {smartSchedule.length > 0 && (
                    ...
                    >
                      ...
                      <div>
                        ...
                        >
                          Ao confirmar, <strong>{smartSchedule.length} eventos sanitários</strong>{' '}
                          serão adicionados ao calendário oficial da fazenda para este{' '}
                  ...
                  <div className="tauze-input-grid grid-col-2 animate-slide-up">
                    <div className="tauze-field-group">
                      ...
                      <input
                        ...
                        placeholder="Ex: Vermifugação de Entrada"
                      />
                    </div>
                    <div className="tauze-field-group">
                            ...
                            <tr key={idx}>
                              ...
                              >
                                <input
                                  type="number"
                                  className="tauze-input"
                                  style={{ padding: '0 8px', height: '36px', textAlign: 'center' }}
                                  value={step.day}
                                  onChange={(e) => {
                              ...
                              >
                                <SearchableSelect
                                  value={(step as any).produto_id || step.product}
                                  onChange={(val) => {
                                    const steps = [...newProtocol.steps];
                                    (steps[idx] as any).produto_id = val;
                                    steps[idx].product =
                                      availableProducts.find((p) => p.value === val)?.label || val;
                                    setNewProtocol({ ...newProtocol, steps });
                                  }}
              ...
              >
                ...
                >
                  <div>
                    ...
                    >
                      {selectedProtocol.id.startsWith('def-')
                        ? 'Copiar & Customizar'
                        : 'Editar Protocolo'}
                    </button>
                  </div>
                      ...
                      selectedProtocol.steps.map((step: any, idx: number) => (
                        <div
                          ...
                          style={{
                            ...
                            position: 'relative',
                            zIndex: 1,
                            marginBottom: idx === selectedProtocol.steps.length - 1 ? 0 : '16px',
                          }}
                        >
                          <div
                            style={{
              ...
              >
                ...
                <p style={{ fontWeight: 700, marginTop: '16px' }}>
                  Selecione um protocolo para detalhamento.
                </p>
              </div>
            )}
          </AnimatePresence>
