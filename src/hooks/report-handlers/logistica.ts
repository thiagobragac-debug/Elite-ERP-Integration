 ...
 */
export const consumoFrotas: ReportHandler = async (
  tenantId,
  fazendaId,
  page = 1,
  pageSize = 20
  ...
  try {
    ...
    return {
      data: (frotaRes.data || []).map((f: any) => ({
        ...
        odometro: f.odometro
          ? `${Number(f.odometro).toLocaleString()} ${f.tipo_odometro || 'km'}`
          : '---',
      ...
      stats: [
        {
          ...
          subtitle: _lastFuelDate
            ? `Último abastecimento: ${_lastFuelDate}`
            : `Período: ${monthYearBR()}`,
 ...
 */
export const manutencoesFrota: ReportHandler = async (
  tenantId,
  fazendaId,
  page = 1,
  pageSize = 20
  ...
  try {
    ...
    const [manutRes, statsRes, maquinasTotalRes, maquinasEmManutRes] = await Promise.all([
      ...
      withTimeout(
        supabase.rpc('get_manutencao_stats', {
          p_tenant_id: tenantId,
          p_fazenda_id: fazendaId,
        }) as unknown as Promise<any>
      ),
    ...
    if (manutRes.error) {
      throw manutRes.error;
    }

    const manut = manutRes.data || [];
    const _lastMaintDate =
      manutRes.data?.length > 0 ? fmtDateBR(manutRes.data[0].data_inicio) : null;
    ...
    return {
      data: manut.map((m: any) => ({
        ...
        status:
          m.status === 'concluido' || m.status === 'Concluído'
            ? 'Concluído ✅'
            : m.status === 'em_andamento' || m.status === 'Em Andamento'
              ? 'Em Andamento ⚙️'
              : `${m.status || 'Agendado'} 🗓️`,
      ...
      stats: [
        {
          ...
          subtitle: _lastMaintDate
            ? `Última manutenção: ${_lastMaintDate}`
            : `Período: ${monthYearBR()}`,
        ...
        },
        {
          ...
          subtitle: _lastMaintDate
            ? `Base: manutenção de ${_lastMaintDate}`
 ...
 */
export const suprimentosInventario: ReportHandler = async (
  tenantId,
  fazendaId,
  page = 1,
  pageSize = 20
  ...
  try {
    ...
    const [invRes, summaryRes] = await Promise.all([
      ...
      withTimeout(
        supabase.rpc('get_inventory_health', {
          p_tenant_id: tenantId,
          p_fazenda_id: fazendaId,
        }) as unknown as Promise<any>
      ),
    ...
    const itensFalta = produtos.filter(
      ...
      stats: [
          ...
          sparkline:
            produtos.length > 0
              ? produtos.map((p: any, i: number) => ({
                  value: Number(
                    (Number(p.estoque_atual || 0) * Number(p.custo_medio || 0)).toFixed(2)
                  ),
                  label: `P${i + 1}`,
                }))
              : [],
        ...
        },
        {
          ...
          sparkline:
            produtos.length > 0
              ? produtos.map((p: any, i: number) => ({
                  value: Number(p.estoque_atual || 0),
                  label: `I${i + 1}`,
                }))
              : [],
 ...
 */
export const pedidosCompra: ReportHandler = async (
  tenantId,
  fazendaId,
  page = 1,
  pageSize = 20
  ...
  try {
    ...
    const [comprasRes, summaryRes] = await Promise.all([
      ...
      withTimeout(
        supabase.rpc('get_purchase_summary', {
          p_tenant_id: tenantId,
          p_fazenda_id: fazendaId,
        }) as unknown as Promise<any>
      ),
      ...
      stats: [
        {
          ...
          subtitle: _lastOrderDate
            ? `Último pedido em ${_lastOrderDate}`
            : `Período: ${monthYearBR()}`,
        ...
        },
        {
          ...
          value:
            pedidosPendentes > 0
              ? pedidosPendentes
              : compras.filter((c: any) => c.status === 'pendente').length || '---',
        ...
        },
        {
          label: 'Média / Pedido',
          subtitle: _lastOrderDate
            ? `Base: pedidos até ${_lastOrderDate}`
            : `Média em ${monthYearBR()}`,
          sparkline: buildSparkline(compras, 'created_at', 'valor_total'),
