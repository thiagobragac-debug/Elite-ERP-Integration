...
// 1. Update interface
content = content.replace('interface NavItem {', 'interface NavItem {\n  permission?: string;');
content = content.replace('subItems?: { title: string; href: string }[];', 'subItems?: { title: string; href: string; permission?: string }[];');

// 2. Replace menuItems with permissioned version
const newMenuItems = const menuItems: NavItem[] = [
...
content = content.replace(/const menuItems: NavItem\[\] = \[[\s\S]*?\];/, newMenuItems);

// 3. Inject useTenant with userProfile and permissions logic
...
// 4. Update the map function in render
content = content.replace('{menuItems.map((item) => (', '{filteredMenuItems.map((item, index) => (');
content = content.replace('<div key={item.title} className="menu-group">', '<div key={index} className="menu-group">');

fs.writeFileSync('C:/Saas/src/components/Sidebar/Sidebar.tsx', content);
console.log("Success!");
