...
import { X, MapPin, Globe, Navigation, Building2, TrendingUp, Search } from 'lucide-react';
...
const getCoord = (sup: any): [number, number] => {
  if (sup.latitude && sup.latitude !== 0) {
    return [sup.latitude, sup.longitude];
  }

  const hash = String(sup.id || sup.nome)
    .split('')
...
export const SupplierNetworkMapModal: React.FC<SupplierNetworkMapModalProps> = ({
  ...
  const uniqueCategories = Array.from(
    new Set(suppliers.map((s) => s.categoria_nome || 'Geral').filter(Boolean))
  );
  ...
  const filteredSuppliers = suppliers.filter((sup) => {
    ...
    const lat =
      sup.latitude !== undefined && sup.latitude !== null && sup.latitude !== 0
        ? sup.latitude
        : null;
        ...
        <div className="map-sidebar">
          ...
          <div className="sidebar-section">
            ...
            <div className="stat-mini">
              ...
              <span className="value" style={{ color: '#0f172a' }}>
                ...
                Object.entries(stateStats)
                  ...
                  .map(([state, count]) => (
                    <div key={state} className="state-item">
                      <span className="s-name" style={{ color: '#475569' }}>
                        {state === 'N/A' ? 'OUT' : state}
                      </span>
                      <div className="s-bar-bg">
                        <div
                          ...
                          style={{
                            width: `${Math.max(10, ((count as number) / suppliers.length) * 100)}%`,
                          }}
                        />
                      </div>
                      <span className="s-count" style={{ color: 'hsl(var(--brand))' }}>
                        {count as number}
                      </span>
                    </div>
                  ))
          ...
          <div className="sidebar-section">
            ...
            <div className="mini-supplier-list">
              ...
              {filteredSuppliers.length > 15 && (
                <div className="more-indicator">+{filteredSuppliers.length - 15} outros</div>
              )}
            </div>
          </div>
            ...
            <MapBounds
              markers={
                showRoute
                  ...
                  : filteredSuppliers.map((sup) => getCoord(sup))
              }
                    ...
                    <div style={{ marginTop: '12px' }}>
                      <button
                        ...
                        style={{
                          ...
                          borderRadius: '4px',
                          border: 'none',
                          background: selectedIds.includes(sup.id)
                            ? '#ef4444'
                            : 'hsl(var(--brand))',
                          color: 'white',
                          fontSize: '10px',
