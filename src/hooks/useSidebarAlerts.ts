...
export const useSidebarAlerts = () => {
  ...
  const [alerts, setAlerts] = useState<SidebarAlertsData>({
    lotes: 0,
    financeiro: 0,
    sanidade: 0,
    configuracoes: 0,
  ...
  const fetchAlerts = async () => {
    ...
    try {
      ...
      let invoicesQuery = supabase
        ...
        .select('id', { count: 'exact', head: true })
        .neq('status', 'pago');
      invoicesQuery = invoicesQuery.eq('tenant_id', activeTenantId);
      ...
      console.warn('[useSidebarAlerts] Suppressed alert counts fetch error:', err);
    } finally {
      setLoading(false);
    }
  };
