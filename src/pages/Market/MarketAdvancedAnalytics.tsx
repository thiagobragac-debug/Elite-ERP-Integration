...
export const MarketAdvancedAnalytics: React.FC = () => {
  ...
  const [showBollinger, setShowBollinger] = usePersistentState(
    'MarketAdvancedAnalytics_showBollinger',
    false
  );
    ...
    try {
      ...
      for (let i = 0; i < rawData.length; i++) {
        ...
        if (current.date >= startDate) {
          ...
          if (!maxPoint || val > maxPoint.value) {
            maxPoint = pt;
          }
          if (!minPoint || val < minPoint.value) {
            minPoint = pt;
          }
        }
      }
  ...
  const handleExport = (format: 'csv' | 'excel' | 'pdf') => {
    ...
    if (format === 'csv') {
      ...
      exportToPDF(exportData, filename, 'Análise Avançada de Mercado');
    }
  };
  ...
  const periodChange =
    currentValue && firstValue ? ((currentValue - firstValue) / firstValue) * 100 : null;
  const volatility =
    data.length > 1
      ? (() => {
  ...
  const fmtVal = (v: number | null) =>
    v !== null ? `${prefix}${v.toFixed(isRatio ? 3 : 2)}${suffix}` : 'â€”';
  ...
  const sparkFromData = (n: number = 7) => {
    ...
    while (pts.length < n && pts.length > 0) {
      pts.unshift(pts[0]);
    }
  ...
  const kpis = [
    {
      ...
      change:
        periodChange !== null
          ? `${periodChange >= 0 ? '+' : ''}${periodChange.toFixed(1)}% no período`
          : 'Carregando...',
      ...
      <header className="page-header">
        <div className="header-brand-group">
          ...
          <p className="page-subtitle">
            Ferramentas de análise gráfica, médias móveis e identificação de topos e fundos
          </p>
        </div>
      </header>
            ...
            <div style={{ display: 'flex', gap: '16px', alignItems: 'center' }}>
              ...
              >
                <input
                  type="checkbox"
                  checked={showMMA7}
                  onChange={(e) => setShowMMA7(e.target.checked)}
              ...
              >
                <input
                  type="checkbox"
                  checked={showMMA30}
                  onChange={(e) => setShowMMA30(e.target.checked)}
            ...
            <div className="export-dropdown-container" style={{ position: 'relative' }}>
              ...
              >
                <button
                  onClick={() => {
                    handleExport('csv');
                    document.getElementById('export-menu-analytics')?.classList.remove('active');
                  }}
                >
                  Excel (.CSV)
                </button>
                ...
                <ComposedChart data={data} margin={{ top: 20, right: 30, left: 0, bottom: 20 }}>
                  ...
                  <Tooltip
                    ...
                    formatter={(val: any, name: any) => {
                      if (name === 'value') {
                        return [
                          `${prefix}${val.toFixed(isRatio ? 3 : 2)}${suffix}`,
                          isRatio ? 'Relação' : 'Cotação Real',
                        ];
                      }
                      if (name === 'mma7') {
                        return [
