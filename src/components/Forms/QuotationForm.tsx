...
export const QuotationForm: React.FC<QuotationFormProps> = ({ isOpen, onClose, onSubmit, initialData }) => {
  ...
  useEffect(() => {
    if (initialData) {
      setFormData({
        ...
        suppliers: initialData.dados_fornecedores || [{ supplier_id: '', price: '', freight: '', delivery_days: '', payment_terms: '', validity: '' }]
      });
    } else {
      setFormData({
        request_id: '',
  ...
  const fetchData = async () => {
    ...
    if (sData) setSuppliersList(sData);
  };

  const addSupplier = () => {
    setFormData({
      ...formData,
      suppliers: [...formData.suppliers, { supplier_id: '', price: '', freight: '', delivery_days: '', payment_terms: '', validity: '' }]
    });
  };
    ...
    >
      <section className="tauze-form-section">
        ...
        <div className="tauze-input-grid grid-col-1" style={{ marginBottom: '16px' }}>
           <div className="tauze-field-group">
            <label className="tauze-label"><ClipboardList size={14} /> Vincular à Solicitação de Compra (Origem)</label>
            <SearchableSelect 
        ...
        <div className="tauze-input-grid grid-col-2">
          ...
          <div className="tauze-field-group">
            <label className="tauze-label"><Hash size={14} /> Quantidade (Base de Cálculo) & Unidade</label>
      ...
      <section className="tauze-form-section">
        <div className="tauze-section-header" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          ...
          <button type="button" className="glass-btn primary" onClick={addSupplier} style={{ fontSize: '11px', height: '32px' }}>
            <Plus size={14} /> NOVO FORNECEDOR
          </button>
        </div>

        <div className="tauze-input-grid grid-col-1">
          <div className="suppliers-list" style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
            {formData.suppliers.map((sup, idx) => {
              ...
              return (
                ...
                >
                  {isWinner && (
                    <div style={{ position: 'absolute', top: '-12px', left: '20px', background: '#10b981', color: '#fff', padding: '4px 12px', borderRadius: '20px', fontSize: '10px', fontWeight: 900, display: 'flex', alignItems: 'center', gap: '4px', letterSpacing: '0.05em' }}>
                      <Trophy size={12} />
                      MENOR CUSTO TCO
                    </div>
                  )}
                  ...
                  <div style={{ display: 'flex', gap: '12px', alignItems: 'flex-end' }}>
                    <div style={{ flex: 1 }}>
                      <label className="tauze-label" style={{ fontSize: '10px' }}>Fornecedor</label>
                      <SearchableSelect 
                        value={sup.supplier_id}
                        onChange={(val: any) => updateSupplier(idx, 'supplier_id', val)}
                        options={[
                          { value: '', label: 'Selecione o Fornecedor...' },
                          ...(suppliersList || []).map(s => ({ value: String(s.id), label: String(s.nome) })),
                        ]}
                      />
                    </div>
                    {formData.suppliers.length > 1 && (
                      <button type="button" onClick={() => removeSupplier(idx)} style={{ height: '42px', width: '42px', display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'hsl(var(--destructive)/0.1)', color: 'hsl(var(--destructive))', border: 'none', borderRadius: '12px', cursor: 'pointer' }}>
                        <Trash2 size={16} />
                      </button>
                    )}
                  </div>
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '12px' }}>
                    <div>
                      <label className="tauze-label" style={{ fontSize: '10px' }}><DollarSign size={12} /> Preço Unitário</label>
                      <input 
                        className="tauze-input"
                        type="number" 
                        step="0.01"
                    ...
                    <div>
                      <label className="tauze-label" style={{ fontSize: '10px' }}><Truck size={12} /> Custo Frete Total</label>
                      <input 
                        className="tauze-input"
                        type="number" 
                        step="0.01"
                    ...
                    <div>
                      <label className="tauze-label" style={{ fontSize: '10px' }}><Calendar size={12} /> Prazo Entrega (Dias)</label>
                      <input 
                        className="tauze-input"
                        type="number" 
                        placeholder="0" 
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: '1.5fr 1fr 1.5fr', gap: '12px', alignItems: 'center', borderTop: '1px dashed hsl(var(--border))', paddingTop: '12px', marginTop: '4px' }}>
                    <div>
                      <label className="tauze-label" style={{ fontSize: '10px' }}><CreditCard size={12} /> Condição de Pagamento</label>
                      <SearchableSelect 
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end', justifyContent: 'center' }}>
                      <span style={{ fontSize: '10px', fontWeight: 800, color: 'hsl(var(--text-muted))', textTransform: 'uppercase' }}>CUSTO TOTAL (TCO)</span>
                      <span style={{ fontSize: '18px', fontWeight: 900, color: isWinner ? '#10b981' : 'hsl(var(--text-main))' }}>
                        {tco !== null ? `R$ ${tco.toLocaleString('pt-BR', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}` : 'R$ 0,00'}
                      </span>
                    </div>
                  </div>
                </motion.div>
