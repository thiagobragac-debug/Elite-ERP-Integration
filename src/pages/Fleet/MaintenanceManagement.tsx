...
export const MaintenanceManagement: React.FC = () => {
  ...
  const [isModalOpen, setIsModalOpen] = usePersistentState(
    'MaintenanceManagement_isModalOpen',
    false
  );
  ...
  const [isHistoryModalOpen, setIsHistoryModalOpen] = usePersistentState(
    'MaintenanceManagement_isHistoryModalOpen',
    false
  );
  const [isChecklistOpen, setIsChecklistOpen] = usePersistentState(
    'MaintenanceManagement_isChecklistOpen',
    false
  );
  const [isPlanModalOpen, setIsPlanModalOpen] = usePersistentState(
    'MaintenanceManagement_isPlanModalOpen',
    false
  );
  ...
  const [showAdvancedFilters, setShowAdvancedFilters] = usePersistentState(
    'MaintenanceManagement_showAdvancedFilters',
    false
  );
  ...
  const handleExport = (format: 'csv' | 'excel' | 'pdf') => {
    const filteredData = orders.filter((o: any) => {
      const matchesSearch =
        ((o.maquinas as any)?.nome || '').toLowerCase().includes(searchTerm.toLowerCase()) ||
        (o.descricao || '').toLowerCase().includes(searchTerm.toLowerCase());
      const isCompleted =
        o.status === 'completed' || o.status === 'CONCLUIDA' || o.status === 'finalizada';
      ...
      const matchesDate =
        (!filterValues.dateStart || new Date(o.data_inicio) >= new Date(filterValues.dateStart)) &&
        (!filterValues.dateEnd || new Date(o.data_inicio) <= new Date(filterValues.dateEnd));

      return (
        matchesSearch && matchesTab && matchesStatus && matchesTypes && matchesCost && matchesDate
      );
    });
    ...
    if (format === 'csv') {
      exportToCSV(exportData, 'log_manutencao');
    } else if (format === 'excel') {
      exportToExcel(exportData, 'log_manutencao');
    } else if (format === 'pdf') {
      exportToPDF(exportData, 'log_manutencao', 'Relatório de Manutenção de Frota');
    }
  };
  ...
  const stats = useMemo(() => {
    ...
    const abertas = orders.filter(
      (o: any) => o.status === 'ABERTA' || o.status === 'open' || o.status === 'pending'
    ).length;
    ...
    const osComData = orders.filter(
      (o: any) => (o.status === 'completed' || o.status === 'CONCLUIDA') && o.data_inicio
    );
    const mttr =
      osComData.length > 0
        ? Math.round(
  ...
  const healthScore = useMemo(() => {
    ...
    if (total === pending) {
      return { score: 0, text: 'Aguardando Inspeção', color: '#64748b' };
    }

    const score = Math.round((passed / total) * 100);
    if (failed > 0) {
      return { score, text: `${failed} Falha(s) Crítica(s)`, color: '#dc2626' };
    }
    if (score === 100) {
      return { score, text: 'Equipamento Saudável', color: '#10b981' };
    }
    return { score, text: 'Inspeção em Andamento', color: '#f59e0b' };
  }, [checklistItems]);

  const isChecklistValid = useMemo(() => {
    if (!checklistMachine || !checklistMeter) {
      return false;
    }
  ...
  const saveMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      toast.success(
        selectedOrder ? 'Ordem de serviço atualizada!' : 'Ordem de serviço aberta com sucesso!'
      );
    },
    onError: (err: any) => {
  ...
  const handleDelete = async (id: string) => {
    const isConfirmed = await confirm({
      title: 'Atenção',
      description: 'Deseja excluir esta ordem de serviço?',
  ...
  const handleViewDetails = (order: any) => {
    ...
    setTimeout(() => {
      setHistoryItems([
        {
          id: '1',
          date: order.data_inicio,
          title: `OS #${order.id.toString().slice(0, 6)}`,
          subtitle: order.descricao,
          value: order.custo_pecas
            ? `R$ ${Number(order.custo_pecas) + Number(order.custo_mao_obra)}`
            : 'N/A',
          status: 'info',
        },
        ...(order.materiais || []).map((m: any, i: number) => ({
          id: `m-${i}`,
          date: order.data_inicio,
          title: `Insumo: ${m.nome || 'Peça'}`,
          subtitle: `Quantidade: ${m.qtd}`,
          value: m.preco ? `R$ ${m.preco * m.qtd}` : 'N/A',
          status: 'success',
        })),
        {
          id: '3',
          date: order.data_inicio,
          title: 'Mão de Obra',
          subtitle: order.responsavel,
          value: order.custo_mao_obra ? `R$ ${order.custo_mao_obra}` : 'CONCLUÍDO',
          status: 'success',
        },
      ]);
    ...
    },
    {
      ...
      accessor: (item: any) => (
        <div style={{ display: 'flex', justifyContent: 'center' }}>
          <span
            className={`status-pill ${item.status === 'completed' ? 'active' : item.status === 'open' ? 'warning' : 'info'}`}
          >
    ...
    <div className="maintenance-page animate-slide-up">
      <header className="page-header">
        <div className="header-brand-group">
          ...
          <p className="page-subtitle">
            Rastreabilidade completa de intervenções mecânicas, revisões preventivas e custos em
            tempo real.
          </p>
        </div>
          ...
          <div className="export-dropdown-container">
            ...
            <div id="export-menu-maint" className="export-menu">
              <button
                onClick={() => {
                  handleExport('csv');
                  document.getElementById('export-menu-maint')?.classList.remove('active');
                }}
              >
                Excel (.CSV)
              </button>
      ...
      <div className="management-content">
        {activeTab === 'PLANS' ? (
            ...
            <div className="animate-fade-in">
              <ModernTable
                ...
                columns={[
                      ...
                      <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                        ...
                        <div style={{ display: 'flex', flexDirection: 'column' }}>
                          ...
                          <span style={{ fontSize: '11px', color: 'hsl(var(--text-muted))' }}>
                            Frequência: {item.freq} {item.unit}
                          </span>
                        </div>
                      </div>
                    ),
                  ...
                  {
                    header: 'Status',
                    accessor: (item: any) => <span className="status-pill active">Ativo</span>,
                    align: 'center',
                  },
                ]}
                loading={false}
          ...
          <div
            className="kanban-board animate-fade-in"
            style={{
              display: 'grid',
          ...
          >
            {[
              ...
              const colOrders = orders.filter((o) => {
                const matchesSearch =
                  (o.maquinas?.nome || '').toLowerCase().includes(searchTerm.toLowerCase()) ||
                  (o.descricao || '').toLowerCase().includes(searchTerm.toLowerCase());
                const matchesTypes =
                  filterValues.types.length === 0 || filterValues.types.includes(o.tipo);
                const totalCost = Number(o.custo_pecas || 0) + Number(o.custo_mao_obra || 0);
                const matchesCost =
                  filterValues.maxCost >= 1000000 || totalCost <= filterValues.maxCost;
                const matchesDate =
                  (!filterValues.dateStart ||
                    new Date(o.data_inicio) >= new Date(filterValues.dateStart)) &&
                  (!filterValues.dateEnd ||
                    new Date(o.data_inicio) <= new Date(filterValues.dateEnd));

                const currentStatus = o.status?.toLowerCase() || 'open';
                const matchesStatus = col.statusKeys
                  .map((k) => k.toLowerCase())
                  ...
                  >
                    <AnimatePresence>
                      {colOrders.length === 0 ? (
                        ...
                        colOrders.map((o) => {
                          const totalCost =
                            Number(o.custo_pecas || 0) + Number(o.custo_mao_obra || 0);
                            ...
                            >
                              ...
                              >
                                ...
                                >
                                  <Calendar size={12} />
                                  {o.data_inicio
                                    ? new Date(o.data_inicio).toLocaleDateString()
                                    : 'N/A'}
                                </span>
                                ...
                                {col.nextStatus && (
                                  <button
                                    ...
                                    style={{
                                      ...
                                      alignItems: 'center',
                                      gap: '4px',
                                      background: col.title.includes('Pendente')
                                        ? '#f59e0b'
                                        : '#3b82f6',
                                      borderColor: col.title.includes('Pendente')
                                        ? '#f59e0b'
                                        : '#3b82f6',
                                      boxShadow: 'none',
                                      height: '28px',
            ...
            data={orders.filter((o) => {
              ...
              const matchesStatus =
                ...
                (filterValues.status === 'open' &&
                  (o.status === 'ABERTA' || o.status === 'pending')) ||
                (filterValues.status === 'completed' && isCompleted);
              const matchesTypes =
                filterValues.types.length === 0 || filterValues.types.includes(o.tipo);
              const totalCost = Number(o.custo_pecas || 0) + Number(o.custo_mao_obra || 0);
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
            <div
              style={{
                ...
                height: '48px',
                borderRadius: '50%',
                background: `${healthScore.color}20`,
            ...
            <div>
              ...
              <div style={{ fontSize: '20px', fontWeight: 900, color: healthScore.color }}>
                {healthScore.score}% - {healthScore.text}
              </div>
            </div>
          </div>
          <div
        ...
        <div className="tauze-input-grid grid-col-2">
          ...
          <div className="tauze-field-group">
            <label className="tauze-label">
              {machines.find((m) => String(m.id) === checklistMachine)?.unidade_medida === 'km' ? (
                <>
                  <Gauge size={14} /> Hodômetro na Inspeção (km)
                </>
              ) : (
                <>
                  <Clock size={14} /> Horímetro na Inspeção (h)
                </>
              )}
            </label>
            <input
                ...
                {item.status === 'fail' && (
                  <div style={{ marginTop: '16px', animation: 'fadeIn 0.3s' }}>
                    ...
                    <textarea
                      ...
                      style={{
                        borderColor: !item.notes.trim() ? '#fca5a5' : 'hsl(var(--border))',
                        background: 'white',
                      }}
                    />
                    {!item.notes.trim() && (
                      ...
                      >
                        A OS não será gerada sem essa informação.
                      </span>
                    )}
                  </div>
                )}
      ...
      >
        ...
        <div className="tauze-field-group">
          ...
          <input
            ...
            className="tauze-input"
            placeholder="Ex: 250"
            defaultValue={selectedPlan?.freq}
          />
        </div>
        ...
        <div className="tauze-field-group" style={{ gridColumn: 'span 2' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
            {(selectedPlan?.items || ['Troca de Óleo', 'Troca de Filtro']).map(
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '8px' }}>
            {['John Deere 7230', 'Patriot 350', 'Ford Cargo'].map((asset) => (
              ...
              >
                <input
                  type="checkbox"
                  defaultChecked={asset === 'John Deere 7230'}
