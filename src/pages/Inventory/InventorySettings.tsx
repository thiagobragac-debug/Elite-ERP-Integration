...
export const NcmSettingsTab: React.FC<{
  searchTerm: string;
  triggerCreate: number;
  triggerImport: number;
}> = ({ searchTerm, triggerCreate, triggerImport }) => {
  ...
  const [isImportModalOpen, setIsImportModalOpen] = usePersistentState(
    'InventorySettings_isImportModalOpen',
    false
  );
  ...
  const saveNcmMutation = useMutation({
    mutationFn: async (payload: any) => {
      if (editItem) {
        const { error } = await supabase.from('estoque_ncms').update(payload).eq('id', editItem.id).eq('tenant_id', tenant?.id);
        if (error) {
          throw error;
        }
      } else {
  ...
  const deleteNcmMutation = useMutation({
    mutationFn: async (id: string) => {
      const { error } = await supabase.from('estoque_ncms').delete().eq('id', id).eq('tenant_id', tenant?.id);
      if (error) {
        throw error;
      }
    },
  ...
  const handleDelete = async (id: string) => {
    const isConfirmed = await confirm({
      title: 'Atenção',
      description: 'Deseja realmente excluir este NCM?',
  ...
  const handleSearchReceita = async (e?: React.FormEvent) => {
    ...
    try {
      const response = await fetch(
        `https://brasilapi.com.br/api/ncm/v1?search=${encodeURIComponent(importSearch)}`
      );
      ...
      console.error(err);
      toast.error(
        'Erro ao buscar na Receita Federal. O serviço pode estar temporariamente indisponível.'
      );
    } finally {
      setImporting(false);
    }
  ...
  const columns = [
    {
      header: 'Código NCM',
      accessor: (ncm: NCM) => (
        <span className="main-text" style={{ fontFamily: 'monospace', fontWeight: 600 }}>
          {ncm.codigo}
        </span>
      ),
      align: 'left' as const,
            ...
            <div className="tauze-field-group" style={{ marginTop: '8px', gridColumn: 'span 2' }}>
              ...
              >
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '2px' }}>
                  ...
                  >
                    Permite usar este NCM nos cadastros de insumo do sistema
                  </span>
                </div>
              </label>
            </div>
              ...
              {importResults.map((res, i) => (
                ...
                >
                  ...
                  >
                    ...
                    <span
                      style={{ fontSize: '13px', color: 'hsl(var(--text-main))', lineHeight: 1.4 }}
                    >
                      {res.descricao}
                    </span>
                  </div>
