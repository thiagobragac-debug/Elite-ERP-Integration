...
export const AuditManagement: React.FC = () => {
  ...
  const [showAdvancedFilters, setShowAdvancedFilters] = usePersistentState(
    'AuditManagement_showAdvancedFilters',
    false
  );
  ...
  const handleExport = (format: 'csv' | 'excel' | 'pdf') => {
    const filteredData = audits.filter((a) => {
      ...
      if (filterValues.accuracyRange === 'excellent') {
        matchesAccuracy = acc >= 98;
      } else if (filterValues.accuracyRange === 'good') {
        matchesAccuracy = acc >= 90 && acc < 98;
      } else if (filterValues.accuracyRange === 'critical') {
        matchesAccuracy = acc < 90;
      }

      const matchesDate =
        (!filterValues.dateStart ||
          new Date(a.data || a.created_at) >= new Date(filterValues.dateStart)) &&
        (!filterValues.dateEnd ||
          new Date(a.data || a.created_at) <= new Date(filterValues.dateEnd));
    ...
    if (format === 'csv') {
      exportToCSV(exportData, 'log_auditorias');
    } else if (format === 'excel') {
      exportToExcel(exportData, 'log_auditorias');
    } else if (format === 'pdf') {
      exportToPDF(exportData, 'log_auditorias', 'Relatório de Auditoria de Estoque');
    }
  };
  ...
  const handleDelete = async (id: string) => {
    const isConfirmed = await confirm({
      title: 'Atenção',
      description: 'Tem certeza que deseja excluir esta auditoria?',
        ...
        >
          ...
          >
            ...
            <span
              style={{
                color:
                  Number(item.accuracy || 0) >= 98
                    ? '#10b981'
                    : Number(item.accuracy || 0) > 90
                      ? '#f59e0b'
                      : '#ef4444',
              }}
            >
      ...
      <header className="page-header">
        <div className="header-brand-group">
          ...
          <p className="page-subtitle">
            Reconciliação física vs. contábil, análise de divergências e controle rigoroso de
            perdas.
          </p>
        </div>
      ...
      <div className="management-content">
        <ModernTable
          emptyState={
            ...
            filterValues.accuracyRange === 'all' &&
            !filterValues.dateStart &&
            !filterValues.dateEnd ? (
              <EmptyState
                title={
                  activeTab === 'HISTORY' ? 'Nenhuma auditoria registrada' : 'Nenhum item crítico'
                }
                description={
                  activeTab === 'HISTORY'
                    ? 'Você não possui histórico de inventário no momento.'
                    : 'Não há auditorias com acuracidade crítica.'
                }
