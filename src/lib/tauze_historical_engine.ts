...
export const generateHistoricalSparkline = async (
  ...
  try {
    ...
    if (kpiType === 'rebanho') {
      ...
      if (error || !data || data.length === 0) return fallbackSparkline();
    ...
    if (kpiType === 'caixa') {
      ...
      if (transacoes.length === 0) return fallbackSparkline();

      for (let i = BUCKETS - 1; i >= 0; i--) {
        const targetDate = subDays(today, i * step);
        let saldo = 0;
        transacoes.forEach(t => { if (t.date <= targetDate) saldo += t.valor; });
        const val = saldo >= 1000 || saldo <= -1000 ? `${(saldo / 1000).toFixed(1)}k` : saldo.toFixed(2);
    ...
    if (kpiType === 'gmd') {
      ...
      if (!data || data.length < 2) return fallbackSparkline();
      ...
      Object.values(byAnimal).forEach(pesagens => {
        for (let i = 1; i < pesagens.length; i++) {
          const days = Math.max(1, (new Date(pesagens[i].data).getTime() - new Date(pesagens[i - 1].data).getTime()) / 86400000);
          const gmd = (pesagens[i].peso - pesagens[i - 1].peso) / days;
          if (!gmdByDate[pesagens[i].data]) gmdByDate[pesagens[i].data] = [];
          gmdByDate[pesagens[i].data].push(gmd);
        }
      });

      const sortedDates = Object.keys(gmdByDate).sort();
      if (sortedDates.length === 0) return fallbackSparkline();
      ...
      for (let i = BUCKETS - 1; i >= 0; i--) {
        ...
        points.push({ value: Math.max(0, avgGMD), label: `${avgGMD.toFixed(3)} kg em ${format(targetDate)}` });
      }
      return points;
    }
    ...
    if (kpiType === 'lotacao') {
      const [animalRes, pastosRes] = await Promise.all([
        supabase.from('animais').select('id', { count: 'exact', head: true }).eq('tenant_id', tenantId).ilike('status', 'ativo'),
        supabase.from('pastos').select('area').eq('tenant_id', tenantId)
      ]);
      const totalAnimais = animalRes.count || 0;
      const totalArea = (pastosRes.data || []).reduce((acc: number, p: any) => acc + Number(p.area || 0), 0);
      const currentLotacao = totalArea > 0 ? totalAnimais / totalArea : 0;

      if (currentLotacao === 0) return fallbackSparkline();

      for (let i = BUCKETS - 1; i >= 0; i--) {
        const targetDate = subDays(today, i * step);
        points.push({ value: currentLotacao, label: `${currentLotacao.toFixed(2)} UA/ha em ${format(targetDate)}` });
      }
      return points;
    }
    ...
    if (kpiType === 'estoque') {
      ...
      const totalValue = (data || []).reduce((acc: number, p: any) => acc + (Number(p.estoque_atual || 0) * Number(p.custo_medio || 0)), 0);
      if (totalValue === 0) return fallbackSparkline();

      for (let i = BUCKETS - 1; i >= 0; i--) {
        const targetDate = subDays(today, i * step);
        const valFormatted = totalValue >= 1000 ? `${(totalValue / 1000).toFixed(1)}k` : totalValue.toFixed(2);
