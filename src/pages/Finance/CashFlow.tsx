...
export const CashFlow: React.FC = () => {
  ...
  const handleSubmit = async (data: any) => {
    if (!canCreate) {
      toast.error(
        '⚠️ Selecione uma unidade específica para lançar uma nova operação. No modo Visão Global, o caixa deve ser vinculado a uma fazenda.'
      );
      return;
    }
  ...
  const handleExport = (format: 'csv' | 'excel' | 'pdf') => {
    const filteredData = transactions.filter((t) => {
      ...
      const matchesTab =
        activeTab === 'ALL' ||
        (activeTab === 'INFLOW' ? t.type === 'inflow' : t.type === 'outflow');
      const matchesStatusFilter =
        statusFilter === 'ALL' ||
        (statusFilter === 'PENDING' ? t.status === 'pending' : t.status === 'paid');

      const matchesType = filterValues.type === 'all' || t.type === filterValues.type;
      const matchesCategory =
        filterValues.categories.length === 0 || filterValues.categories.includes(t.category);
      const matchesLiquidation =
        filterValues.status === 'all' ||
        t.status === (filterValues.status === 'PAID' ? 'paid' : 'pending');

      const txDate = new Date(t.date);
      const matchesDateStart =
        !filterValues.dateStart || txDate >= new Date(filterValues.dateStart);
    ...
    if (format === 'csv') {
      exportToCSV(exportData, 'fluxo_caixa');
    } else if (format === 'excel') {
      exportToExcel(exportData, 'fluxo_caixa');
    } else if (format === 'pdf') {
      exportToPDF(exportData, 'fluxo_caixa', 'Relatório de Fluxo de Caixa');
    }
  };
    ...
    setHistoryItems([
      {
        id: '1',
        date: tx.date,
        title: `Transação: ${tx.description}`,
        subtitle: `Categoria: ${tx.category}`,
        value: tx.amount.toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' }),
        status: tx.type === 'inflow' ? 'success' : 'warning',
      },
      {
        id: '2',
        date: tx.date,
        title: 'Status Financeiro',
        subtitle: tx.status === 'paid' ? 'Liquidado' : 'Provisionado',
        value: tx.status.toUpperCase(),
        status: tx.status === 'paid' ? 'success' : 'pending',
      },
    ]);
  };
      ...
      accessor: (item: any) => (
        <div
          style={{
            ...
            textAlign: 'left',
            opacity: item.status === 'pending' ? 0.6 : 1,
            fontStyle: item.status === 'pending' ? 'italic' : 'normal',
          }}
        >
      ...
      accessor: (item: any) => (
        <div
          style={{
            ...
            gap: '4px',
            fontWeight: 800,
            color: item.type === 'inflow' ? '#059669' : '#e11d48',
            opacity: item.status === 'pending' ? 0.6 : 1,
          }}
        >
      ...
      accessor: (item: any) => {
        ...
        return (
          ...
          >
            ...
            <span
              style={{
                background: isNegative ? '#fee2e2' : 'transparent',
                padding: isNegative ? '2px 8px' : '0',
                borderRadius: '4px',
              }}
            >
    ...
    },
    {
      ...
      accessor: (item: any) => (
        <div style={{ display: 'flex', justifyContent: 'center' }}>
          <span
            className={`status-pill ${item.status === 'paid' ? 'active' : 'warning'}`}
            style={{ opacity: item.status === 'pending' ? 0.6 : 1 }}
          >
    ...
    <div className="cash-flow-page animate-slide-up">
      <header className="page-header">
        <div className="header-brand-group">
          <Breadcrumb
            paths={[
              { label: 'Financeiro & Banco', href: '/financeiro' },
          ...
          <p className="page-subtitle">
            Gestão operacional e inteligência financeira avançada em um único dashboard.
          </p>
        </div>
      ...
      <div className="operational-view-content animate-slide-up" style={{ animationDelay: '0.1s' }}>
        <div className="tauze-controls-row">
          <div className="tauze-tab-group">
            ...
            <button
              className={`tauze-tab-item ${activeTab === 'INFLOW' ? 'active' : ''}`}
              onClick={() => setActiveTab('INFLOW')}
            >
              Entradas
            </button>
            <button
              className={`tauze-tab-item ${activeTab === 'OUTFLOW' ? 'active' : ''}`}
          ...
          <div
            className="tauze-tab-group secondary"
            style={{ marginLeft: '12px', background: 'hsl(var(--bg-main)/0.5)', padding: '2px' }}
          >
            <button
            ...
            <div className="export-dropdown-container">
              ...
              <div id="export-menu-cashflow" className="export-menu">
                <button
                  onClick={() => {
                    handleExport('csv');
                    document.getElementById('export-menu-cashflow')?.classList.remove('active');
                  }}
                >
                  Excel (.CSV)
                </button>
      ...
      <HistoryModal
        ...
        subtitle="Rastreabilidade completa"
        items={historyItems}
      />
