...
describe('useSidebarAlerts hook', () => {
  ...
  it('should fetch and compute correct counts of alerts', async () => {
    ...
    (supabase.from as any).mockImplementation((table: string) => {
      if (table === 'lotes') return { select: vi.fn().mockReturnValue({ in: mockIn }) };
      if (table === 'contas_pagar' || table === 'contas_receber') return { select: vi.fn().mockReturnValue({ neq: mockNeq }) };
      if (table === 'sanidade') return { select: vi.fn().mockReturnValue({ eq: mockEq }) };
      if (table === 'saas_invoices') return { select: vi.fn().mockReturnValue({ neq: mockNeq }) };
